...
def test_phase6_stale_context_claim_handler_uses_one_committed_proof(
    tmp_path,
):
    session_factory = create_session_factory(tmp_path / "research.db")
    incident = record_runtime_incident(
    ...
    class ContextClaimRecovery:
        ...
        def consume_verification(self, *, incident_id):
            if any(call[0] == "verify" for call in calls):
                return None
            if not any(call[0] == "recover" for call in calls):
                return None
            calls.append(("verify", incident_id))
            return {
                "applicable": True,
                ...
                "context_attempt_id": 41,
            }

    recovery = ContextClaimRecovery()
    tools = _build_runtime_agent_cli_tools(
        session_factory,
        max_output_bytes=8192,
        journal_reader=lambda: (),
        context_claim_recovery=recovery,
    )
    handlers = _build_runtime_agent_action_handlers(
        tools,
        context_claim_recovery=recovery,
    ...
    assert handlers["recover_stale_side_effect_free_claim"](
        incident_id=incident.id,
        idempotency_key="runtime-incident:1:recover-claim:v1",
        expected_fingerprint=incident.fingerprint,
    )
    verified = tools.execute(
        "get_worker_state",
    ...
    assert verified.data == {
        "incident_id": incident.id,
        "worker_kind": "context_resolution_attempt",
        "applicable": True,
        "safe_queue_restored": True,
        "claim_status": "pending",
        "business_write_owned": False,
    ...
    assert passive.data["applicable"] is False
    assert calls[-1] == ("verify", incident.id)
