...
def test_lifespan_listener_owns_exactly_one_telegram_disconnect(tmp_path):
    ...
    async def exercise_lifespan():
        ...
        async with app.router.lifespan_context(app):
            await asyncio.wait_for(client.listener_started.wait(), timeout=1.0)
        return client.disconnect_calls

    assert asyncio.run(exercise_lifespan()) == 1
...
def test_mimo_full_settings_payload_cannot_reverse_concurrent_ingest_transition(
    ...
    app = create_web_app(
        database_path=tmp_path / "mimo-full-payload-interleaving.db",
        runtime_role="web",
    )
    real_save = web_app_module.save_trading_settings
    interleavings = []

    def save_after_ingest_transition(session_factory, payload, **kwargs):
        if not interleavings:
            web_app_module.transition_message_concurrency_settings(
                session_factory,
                {
            ...
            interleavings.append("ingest_cutover_committed")
        return real_save(session_factory, payload, **kwargs)

    monkeypatch.setattr(
        web_app_module,
        "save_trading_settings",
        save_after_ingest_transition,
    )
    ...
    assert response.status_code == 200
    assert interleavings == ["ingest_cutover_committed"]
    final = web_app_module.load_trading_settings(app.state.session_factory)
    assert (
...
def test_web_full_settings_payload_remains_unrelated_when_cutover_precedes_second_read(
    ...
    async def requester(url, *, payload, timeout_seconds):
        proxy_calls.append((url, payload, timeout_seconds))
        return httpx.Response(500, json={"detail": "unexpected proxy"})

    app = create_web_app(
        database_path=tmp_path / "web-second-read-interleaving.db",
        runtime_role="web",
        ingest_trading_settings_requester=requester,
    )
    real_load = web_app_module.load_trading_settings
    load_calls = 0
    interleavings = []

    def load_with_cutover_before_second_read(session_factory):
        nonlocal load_calls
        load_calls += 1
        if load_calls == 2:
            web_app_module.transition_message_concurrency_settings(
            ...
            interleavings.append("cutover_before_second_read")
        return real_load(session_factory)

    monkeypatch.setattr(
        web_app_module,
        "load_trading_settings",
        load_with_cutover_before_second_read,
    )
    ...
    assert response.status_code == 200
    assert interleavings == ["cutover_before_second_read"]
    assert proxy_calls == []
...
@pytest.mark.parametrize(
    ...
    [
        (
            "/api/execution/close-bound-position",
            {"pos_id": "", "extra": "forbidden"},
            400,
            "only pos_id is accepted",
        ),
        (
            "/api/execution/close-bound-position",
            {"pos_id": ""},
        ...
        (
            "/api/recovery-live-submit",
            {"chat_id": 100, "message_id": 55, "symbol": "", "side": "long"},
            422,
            "missing required fields: symbol",
        ),
    ],
)
def test_worker_command_route_validation_contract_precedes_exchange_factory(
    tmp_path, path, payload, expected_status, expected_detail
):
    factory_calls = []
    app = create_web_app(
        database_path=tmp_path / "research.db",
        deepcoin_client_factory=lambda: factory_calls.append(True),
    )
