...
def test_shadow_replay_honors_intervening_entry_boundary(tmp_path):
    database = tmp_path / "replay-boundary.db"
    connection = sqlite3.connect(database)
    connection.executescript(
        """
        CREATE TABLE raw_messages (id INTEGER PRIMARY KEY, chat_id INTEGER, message_id INTEGER, posted_at TEXT);
        CREATE TABLE entry_preambles (id INTEGER PRIMARY KEY, raw_message_id INTEGER, chat_id INTEGER, message_id INTEGER, symbol TEXT, side TEXT, risk_multiplier TEXT, status TEXT);
        CREATE TABLE signal_candidates (id INTEGER PRIMARY KEY, raw_message_id INTEGER, symbol TEXT, side TEXT, event_type TEXT);
        INSERT INTO raw_messages VALUES
          (9334, 9, 9901, '2026-08-05 01:00:00'),
          (9336, 9, 99015, '2026-08-05 01:00:30'),
          (9335, 9, 9902, '2026-08-05 01:01:00');
        INSERT INTO entry_preambles VALUES (1, 9334, 9, 9901, 'BTCUSDT', 'long', '0.5', 'pending');
        INSERT INTO signal_candidates VALUES
          (1, 9336, 'BTCUSDT', 'long', 'entry_signal'),
          (2, 9335, 'BTCUSDT', 'long', 'entry_signal');
        """
    )
    connection.commit()
    connection.close()
    ...
    completed = subprocess.run(
        [sys.executable, str(script), "--database-path", str(database),
         "--preamble-raw-message-id", "9334", "--strategy-raw-message-id", "9335",
    ...
    assert payload["preamble_message_id"] is None
    assert payload["proposed_effective_risk_budget_usdt"] == "20"


def test_shadow_replay_honors_intervening_cancel_management_action(tmp_path):
    database = tmp_path / "replay-cancel-boundary.db"
    connection = sqlite3.connect(database)
    connection.executescript(
        """
        CREATE TABLE raw_messages (id INTEGER PRIMARY KEY, chat_id INTEGER, message_id INTEGER, posted_at TEXT);
        CREATE TABLE entry_preambles (id INTEGER PRIMARY KEY, raw_message_id INTEGER, chat_id INTEGER, message_id INTEGER, symbol TEXT, side TEXT, risk_multiplier TEXT, status TEXT);
        CREATE TABLE signal_candidates (id INTEGER PRIMARY KEY, raw_message_id INTEGER, symbol TEXT, side TEXT, event_type TEXT, management_action TEXT);
        INSERT INTO raw_messages VALUES
          (9334, 9, 9901, '2026-08-05 01:00:00'),
          (9336, 9, 99015, '2026-08-05 01:00:30'),
          (9335, 9, 9902, '2026-08-05 01:01:00');
        INSERT INTO entry_preambles VALUES (1, 9334, 9, 9901, 'BTCUSDT', 'long', '0.5', 'pending');
        INSERT INTO signal_candidates VALUES
          (1, 9336, 'BTCUSDT', 'long', 'unknown', 'cancel_entry'),
          (2, 9335, 'BTCUSDT', 'long', 'entry_signal', NULL);
        """
    )
    connection.commit()
    connection.close()
    ...
    assert json.loads(completed.stdout)["decision"] == "none"
