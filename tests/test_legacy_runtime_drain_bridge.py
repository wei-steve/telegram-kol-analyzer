...
def test_bridge_plan_blocks_prior_unknown_reviewed_cancel(tmp_path):
    session_factory = create_session_factory(tmp_path / "unknown-plan.db")
    _seed_unknown_reviewed_cancel(session_factory, REVIEWED_IDS[0])

    plan = _absent_plan(session_factory)

    assert plan.conflicts == (
        {"reason": "legacy_bridge_unknown_mutation_present"},
    )


@pytest.mark.parametrize("order_id", (REVIEWED_IDS[0], "historical-unrelated"))
def test_bridge_plan_scopes_terminal_revision_child_ambiguity(
    tmp_path,
    order_id,
):
    session_factory = create_session_factory(tmp_path / f"revision-{order_id}.db")
    with session_factory() as session:
        raw = RawMessage(chat_id=701, message_id=702, text="historical")
        ...
        batch = StrategyRevisionBatch(
            ...
            target_lifecycle_id=8001,
            execution_binding_id=8002,
...
def test_rollback_refuses_governed_setting_drift(tmp_path):
    ...
    result = rollback_legacy_runtime_drain_bridge(
        ...
        runtime_identity=_identity(),
        confirmation_token="rollback-drift-token",
        rolled_back_at=NOW,
    )

    assert result.status == "blocked"
    assert result.reason_code == "legacy_bridge_auto_trade_not_frozen"
    assert _stored_bridge(session_factory)["state"] == "fenced"


def test_rollback_refuses_unknown_reviewed_cancel(tmp_path):
    session_factory = create_session_factory(tmp_path / "rollback-unknown.db")
    bridge_token = _fenced_bridge(session_factory)
    _seed_unknown_reviewed_cancel(session_factory, REVIEWED_IDS[0])
    ...
    result = rollback_legacy_runtime_drain_bridge(
        ...
        bridge_token=bridge_token,
        runtime_identity=_identity(),
        confirmation_token="rollback-unknown-token",
        rolled_back_at=NOW,
    )
...
def _seed_unknown_reviewed_cancel(session_factory, order_id):
    with session_factory() as session:
        session.add(
            PositionMutationIntent(
                idempotency_key=f"unknown:{order_id}",
                venue="deepcoin",
                operation="cancel_reviewed_pending_entry",
                strategy_instance_id=f"unknown:{order_id}",
...
def _seed_terminal_local_drain_state(session_factory):
    with session_factory() as session:
        for index, order_id in enumerate(REVIEWED_IDS, start=1):
            ...
            stored_request = {"source": order_id}
            cancel_request = {
                "instId": "BTC-USDT-SWAP",
                "ordId": order_id,
            }
            request_json_fingerprint = hashlib.sha256(
                json.dumps(
                    stored_request,
                    ensure_ascii=True,
                    sort_keys=True,
            ...
            event_before = {
                "action_id": "e" * 64,
                "exchange_row_fingerprint": "f" * 64,
                "plan_fingerprint": "a" * 64,
            }
            authority_material = {
                **event_before,
                "request_json_fingerprint": request_json_fingerprint,
            }
            ...
            cancel_request_fingerprint = hashlib.sha256(
                json.dumps(
                    cancel_request,
                    ensure_ascii=True,
                    sort_keys=True,
            ...
            session.add(
                ExecutionBinding(
                    ...
                    venue="deepcoin",
                    status="cancelled",
                    last_exchange_status="reviewed_pending_entries_cancelled",
                )
            )
            session.add(
                StrategyLifecycle(
                    ...
                    management_action="reviewed_pending_entries_cancelled",
                )
            )
            session.add(
                ExecutionOrderLeg(
                    ...
                    request_json=json.dumps(stored_request, sort_keys=True),
                )
            )
            session.add(
                PositionMutationIntent(
                    ...
                    request_fingerprint=cancel_request_fingerprint,
                    status="confirmed",
                    request_json=json.dumps(cancel_request, sort_keys=True),
                    response_json=json.dumps(
                        {"code": "0", "order_id": order_id},
                        sort_keys=True,
                    ),
                    reserved_at=NOW,
                    submitted_at=NOW,
            ...
            session.add(
                ExecutionEvent(
                    ...
                    before_json=json.dumps(event_before, sort_keys=True),
...
def test_drain_rechecks_exact_local_event_evidence_inside_transaction(tmp_path):
    session_factory = create_session_factory(tmp_path / "drain-event-drift.db")
    bridge_token = _fenced_bridge(session_factory)
    _complete_all_bridge_orders(session_factory, bridge_token)
    _seed_terminal_local_drain_state(session_factory)
    evidence = _complete_drain_evidence()
    with session_factory() as session:
        event = (
            ...
            .filter(ExecutionEvent.order_id == REVIEWED_IDS[0])
            .one()
        )
        event.before_json = "{}"
        event.response_json = '{"credential":"must-not-count"}'
        session.commit()
    ...
    result = mark_legacy_runtime_bridge_drained(
        ...
        runtime_identity=_identity(),
        evidence=evidence,
        confirmation_token="drain-event-drift-token",
        drained_at=NOW,
    )

    assert result.status == "blocked"
    assert result.reason_code == "legacy_bridge_local_state_incomplete"
    assert _stored_bridge(session_factory)["state"] == "fenced"


def test_drain_rechecks_complete_binding_terminalization_inside_transaction(
    tmp_path,
):
    session_factory = create_session_factory(tmp_path / "drain-binding-drift.db")
    bridge_token = _fenced_bridge(session_factory)
    _complete_all_bridge_orders(session_factory, bridge_token)
    _seed_terminal_local_drain_state(session_factory)
    evidence = _complete_drain_evidence()
    with session_factory() as session:
        binding = session.get(ExecutionBinding, 2001)
        binding.status = "open"
        session.commit()
    ...
    result = mark_legacy_runtime_bridge_drained(
        ...
        runtime_identity=_identity(),
        evidence=evidence,
        confirmation_token="drain-binding-drift-token",
        drained_at=NOW,
    )
...
def test_local_runtime_identity_rechecks_service_main_pid(tmp_path):
    ...
    def runner(argv, **_kwargs):
        ...
        if argv[-2:] == ["rev-parse", "--show-toplevel"]:
            ...
            pid_reads += 1
            output = "4321" if pid_reads == 1 else "4322"
        else:
            raise AssertionError(argv)
        return subprocess.CompletedProcess(argv, 0, output + "\n", "")

    with pytest.raises(ValueError, match="MainPID"):
        read_local_legacy_worker_identity(
