...
def test_legacy_bridge_null_time_sentinel_never_becomes_stale(tmp_path):
    from telegram_kol_research.entry_revision_executor import execute_entry_revision
    ...
    session_factory = create_session_factory(tmp_path / "legacy-sentinel.db")
    batch_id = _planned_batch(session_factory)
    save_trading_settings(
        session_factory,
        {
    ...
    reviewed_ids = tuple(row.order_id for row in REVIEWED_PENDING_ENTRY_TARGETS)
    ...
    frozen = freeze_legacy_runtime_drain_bridge(
        ...
        confirmation_token="sentinel-freeze-token",
        frozen_at=NOW,
    )
    ...
    with session_factory() as session:
        batch = session.get(StrategyRevisionBatch, batch_id)
        assert batch.advance_claim_token
        assert batch.advance_claimed_at is None
    save_trading_settings(
        session_factory,
        {"entry_revision_v2_mode": "live"},
    ...
    assert result.status == "in_progress"
    assert result.reason_code == "entry_revision_already_claimed"
    assert client.events == []
