...
from telegram_kol_research.web_queries import (
    _candidate_matches_mimo_intent,
    load_group_messages,
)
...
def test_web_projection_keeps_exhausted_v2_failure_distinct_from_projection_failure(
    ...
    assert analysis["format"] == "v2"
    assert analysis["runtime"] is None
    assert analysis["authoritative_runtime"] is None
    assert analysis["latest_call"]["status"] == "failed"
    assert analysis["latest_call"]["error_code"] == "provider_timeout"
...
def test_web_projection_keeps_running_mimo_distinct_from_success(tmp_path):
    ...
    assert analysis["runtime"] is None
    assert analysis["authoritative_runtime"] is None
    assert analysis["latest_call"]["status"] == "running"
    assert analysis["latest_call"]["status_label"] == "MiMo识别进行中"
...
def test_web_projection_rejects_candidate_with_different_action_parameters(tmp_path):
    factory = create_session_factory(tmp_path / "parameter-mismatch.db")
    with factory() as session:
        raw = RawMessage(chat_id=88, message_id=49, text="move stop")
        session.add(raw)
        session.flush()
        run = MimoRecognitionRun(
            raw_message_id=raw.id,
            ...
            model="mimo-v2.5",
            input_kind="text",
            input_fingerprint="sha256:parameter-mismatch",
            prompt_versions_json="{}",
            status="completed",
            attempt_count=1,
            selected_attempt_ordinal=1,
        ...
        _add_decision(session, raw, status="skipped", reason="not_projected")
        session.commit()
    ...
    assert acceptance["status"] == "not_accepted"
    assert acceptance["intents"][0]["status"] == "rejected"


def test_web_projection_rejects_revision_candidate_with_strategy_drift():
    candidate = SignalCandidate(
        raw_message_id=1,
        symbol="ETH",
        side="short",
        ...
        entry_text="1880-1890",
        stop_loss_text="1850",
        take_profit_text="1950",
        leverage_text="20",
        parse_source="mimo_authoritative",
        confidence=0.95,
    )

    assert not _candidate_matches_mimo_intent(
        candidate,
        {
            "kind": "replace_entry",
            "target": {"lifecycle_id": 790, "thread_id": 52},
...
def test_web_keeps_current_authority_but_exposes_latest_failed_rerun(tmp_path):
    factory = create_session_factory(tmp_path / "stale-success-latest-failure.db")
    with factory() as session:
        raw = RawMessage(chat_id=88, message_id=46, text="BTC long")
        session.add(raw)
        session.flush()
        ...
        latest = MimoRecognitionRun(
            raw_message_id=raw.id,
            run_kind="v2_authoritative",
            contract_version="mimo-authoritative-v2",
            model="mimo-v2.5",
            input_kind="text",
            input_fingerprint="sha256:rerun",
            prompt_versions_json="{}",
            status="failed",
            attempt_count=1,
            final_error_code="provider_timeout",
    ...
    assert analysis["format"] == "v1"
    assert analysis["summary"] == "BTC多单"
    assert analysis["authoritative_runtime"]["status"] == "completed"
    assert analysis["runtime"] == analysis["authoritative_runtime"]
    assert analysis["latest_call"]["run_id"] == latest_id
    assert analysis["latest_call"]["status"] == "failed"
    assert analysis["latest_call"]["error_code"] == "provider_timeout"


def test_web_keeps_current_authority_while_latest_rerun_is_running(tmp_path):
    factory = create_session_factory(tmp_path / "authority-latest-running.db")
    with factory() as session:
        raw = RawMessage(chat_id=88, message_id=47, text="market note")
        session.add(raw)
        session.flush()
        authoritative = MimoRecognitionRun(
            raw_message_id=raw.id,
            ...
            model="mimo-v2.5",
            input_kind="text",
            input_fingerprint="sha256:authority-v2",
            prompt_versions_json='{"trading.analysis.mimo_v2_authoritative":1}',
            status="completed",
            attempt_count=1,
            selected_attempt_ordinal=1,
        ...
        session.add(authoritative)
        session.flush()
        _add_v2_evidence(session, raw, _v2_payload(), authoritative)
        running = MimoRecognitionRun(
            raw_message_id=raw.id,
            run_kind="v2_authoritative",
            contract_version="mimo-authoritative-v2",
            model="mimo-v2.5",
            input_kind="text",
            input_fingerprint="sha256:running-rerun",
            prompt_versions_json="{}",
            status="running",
            attempt_count=0,
            became_authoritative=False,
    ...
    assert analysis["format"] == "v2"
    assert analysis["authoritative_runtime"]["status"] == "completed"
    assert analysis["latest_call"]["run_id"] == running_id
    assert analysis["latest_call"]["status"] == "running"
    assert analysis["intents"][0]["intent_type"] == "position_management"


def test_web_projection_preserves_zero_confidence_without_free_text_parsing(
    tmp_path,
    monkeypatch,
):
    factory = create_session_factory(tmp_path / "zero-confidence.db")
    with factory() as session:
        raw = RawMessage(chat_id=88, message_id=48, text="punctuation, should not matter!")
        session.add(raw)
        session.flush()
        run = MimoRecognitionRun(
            raw_message_id=raw.id,
        ...
        session.add(
            MessageEvidenceVersion(
                ...
                extraction_status="completed",
                confidence=0.0,
                text_evidence_json='{"observed_text":"literal"}',
                image_evidence_json="{}",
                normalized_evidence_json=(
                    '{"recognition_result":"非策略","summary":"unclear",'
                    '"confidence":0.0,"strategy":{}}'
                ),
            )
        )
        session.commit()
    monkeypatch.setattr(
        "telegram_kol_research.web_queries.re.fullmatch",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("MiMo projection must not parse free text")
        ),
    )
