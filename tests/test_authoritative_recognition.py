...
def _v1_nontrading_result(raw_message_id: int) -> MimoAuthoritativeResult:
    return MimoAuthoritativeResult(
        ...
        payload={
            ...
            "evidence": {
                "text": {"observed_text": "ordinary commentary", "fields": {}},
                "images": [],
                "conflicts": [],
            },
        },
...
def test_mimo_v1_remains_default_and_v2_is_not_called(tmp_path, monkeypatch):
    ...
    assert evidence.mimo_recognition_run_id == run.id


def test_mimo_v2_runs_only_above_future_watermark(tmp_path, monkeypatch):
    session_factory = create_session_factory(tmp_path / "mimo-watermark.db")
    with session_factory() as session:
        before = RawMessage(chat_id=91, message_id=1, text="ordinary commentary")
        after = RawMessage(chat_id=91, message_id=2, text="ordinary commentary")
    ...
    save_trading_settings(
        session_factory,
        {
            ...
            "mimo_v2_activation_after_raw_message_id": before_id,
        },
    )
    v2_calls: list[int] = []
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "telegram_kol_research.authoritative_recognition.run_mimo_authoritative_for_message",
        lambda *args, **kwargs: _v1_nontrading_result(kwargs["raw_message_id"]),
    )

    before_result = process_authoritative_message(
        session_factory,
        raw_message_id=before_id,
        ai_recognition_config=AiRecognitionConfig(),
        media_root=tmp_path,
    )
    after_result = process_authoritative_message(
        session_factory,
        raw_message_id=after_id,
        ai_recognition_config=AiRecognitionConfig(),
        media_root=tmp_path,
    )

    assert before_result.assessment.mimo.contract_version == "v1"
    assert after_result.assessment.mimo.contract_version == "mimo-authoritative-v2"
    assert v2_calls == [after_id]
    with session_factory() as session:
        evidence = (
            session.query(MessageEvidenceVersion)
            .filter(MessageEvidenceVersion.raw_message_id == after_id)
            .one()
        )
        run = session.get(MimoRecognitionRun, evidence.mimo_recognition_run_id)
    assert evidence.mimo_recognition_run_id == after_result.assessment.mimo.run_id
    assert run is not None
    assert run.projection_fingerprint == (
        after_result.assessment.mimo.projection_fingerprint
    )
...
def test_mimo_v2_contract_failure_falls_back_once_before_execution_claim(
    ...
    monkeypatch.setattr(
        ...
        or _failed_v2_inference(session_factory, raw_id),
        raising=False,
    )
    monkeypatch.setattr(
        "telegram_kol_research.authoritative_recognition.run_mimo_authoritative_for_message",
    ...
    assert (
        runs_by_kind["v1_fallback"].retry_of_run_id
...
def test_open_mimo_v2_circuit_routes_later_message_to_v1(tmp_path, monkeypatch):
    ...
    def fail_v2(*args, **kwargs):
        v2_calls.append(kwargs["raw_message_id"])
        return _failed_v2_inference(
            session_factory,
            kwargs["raw_message_id"],
        )
...
def test_mimo_v2_never_falls_back_after_execution_claim(tmp_path, monkeypatch):
