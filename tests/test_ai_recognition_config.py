...
def test_default_trading_templates_have_strict_boundaries():
    ...
    assert "reply_context" in DEFAULT_LIFECYCLE_EVENT_PROMPT
    assert "精确 Telegram 回复目标" in DEFAULT_LIFECYCLE_EVENT_PROMPT
    assert "不得自动转为 exit_position" in DEFAULT_LIFECYCLE_EVENT_PROMPT
    ...
    assert "reply_context" in DEFAULT_SHARED_TRADING_ANALYSIS_PROMPT
    assert '"entry_fragments"' in DEFAULT_SHARED_TRADING_ANALYSIS_PROMPT
    assert "配置最大亏损预算" in DEFAULT_SHARED_TRADING_ANALYSIS_PROMPT
