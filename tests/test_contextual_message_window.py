...
def test_context_serializes_saved_text_and_per_image_evidence_without_pixels(tmp_path):
    session_factory = create_session_factory(tmp_path / "v2-context.db")
    with session_factory() as session:
        prior = RawMessage(
            chat_id=71,
            message_id=710,
    ...
    save_message_evidence_version(
        session_factory,
        raw_message_id=prior_id,
        input_fingerprint="sha256:v2-context",
        model="mimo-v2.5",
        prompt_versions={"mimo_v2": 12},
        extraction_status="completed",
        confidence=0.97,
        text_evidence={"observed_text": "", "fields": {}},
        image_evidence={
            "images": [
                {
                    "asset_id": 381,
    ...
    with session_factory() as session:
        window = build_contextual_message_window(session, raw_message_id=current_id)

    prior_context = window.messages[0]
    assert prior_context.text_evidence == {"observed_text": "", "fields": {}}
    assert prior_context.image_evidence["images"][0]["asset_id"] == 381
    assert prior_context.image_evidence["images"][0]["summary"] == "ETHUSDT空仓持仓截图"
    rendered = render_authoritative_context(window)
    assert '"image_evidence"' in rendered
    assert "ETHUSDT空仓持仓截图" in rendered
    assert "data:image" not in rendered


def test_context_keeps_historical_v1_evidence_shape_without_inventing_v2(tmp_path):
    session_factory = create_session_factory(tmp_path / "v1-context.db")
    with session_factory() as session:
        prior = RawMessage(chat_id=72, message_id=720, posted_at=_at(1), text="BTC做多")
        current = RawMessage(chat_id=72, message_id=721, posted_at=_at(2), text="继续持有")
        session.add_all([prior, current])
        session.commit()
        prior_id = prior.id
        current_id = current.id
    save_message_evidence_version(
        session_factory,
        raw_message_id=prior_id,
        input_fingerprint="sha256:v1-context",
        model="mimo-v2.5",
        prompt_versions={"evidence": "v1"},
        extraction_status="completed",
        confidence=0.9,
        text_evidence={"observed_text": "BTC做多", "fields": {}},
        image_evidence={"images": []},
        normalized_evidence={"recognition_result": "是策略", "symbol": "BTC"},
    )

    with session_factory() as session:
        window = build_contextual_message_window(session, raw_message_id=current_id)

    historical = window.messages[0]
    assert historical.normalized_evidence == {
        "recognition_result": "是策略",
        "symbol": "BTC",
    }
    assert "contract_version" not in historical.normalized_evidence
    assert historical.image_evidence == {"images": []}
