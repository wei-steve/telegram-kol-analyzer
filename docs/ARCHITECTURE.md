...
- **挂在"尚未成交的限价入场单"上的止损要排除掉，它不是任何仓位的保护单。**
  ...
  **"排除"只是权威链的答案，不是页面该说的话**（2026-09-23 更正）。这条规则成形于
  REST-only 时期：那时随委托单一起挂上的止损查不清归属，又不敢不带止损，于是先带上、
  成交后再撤掉重设，而"查不清"的那一半就只能靠排除来兜底。
  **REST+WS 之后这个前提不成立了**：`TU` 由交易所直说仓位存不存在，
  四元组匹配的又是我们**自己**的持久腿记录，两者合起来足以说出这张止损是**哪一条**
  挂单入场腿带上去的——2026-09-23 实盘四条挂单腿，签名两两不同，逐条对得上。
  所以页面按 `protection_authority.resting_entry_stop_owners()` 把它们单列为
  **「挂单入场自带止损（入场尚未成交）」**并标出群与腿，不再和真正的无主单混在
  「未归属交易所保护单」里。两条腿签名完全相同时不猜，退回未归属。
  **权威链这一侧的行为不变**：仓位还不存在，这张止损就不是任何仓位的保护，
  仍然不进保护集合、不冻结、不撤。变的只是页面不再把"知道是谁的"报成"不知道是谁的"。
...
- **"只向前"要对着已经存在的终态测，而不是只对着转变测。**
  2026-09-12（A-17）：十个关闭点各有用例、遍历守卫计数、变异检查逐个转红——**全部从"尚未关闭"出发**。
  生产上第一轮对账就把 83 个早已关闭的 binding 重新推导了一遍，664 行历史被终态化。
  **"写入点"不等于"状态转变点"**：同一行 `binding.status = "closed"`，对 active 的 binding 是转变，
  对已关闭的 binding 是重述；对账、修复、回放这类**周期性重推导**的路径，每一轮都在重述。
  **做法**：凡是挂在状态赋值上的副作用，至少一条用例从目标状态出发、跑一整轮真实调用方，断言副作用不发生。
- **变异的还原方式不能碰被测的改动本身。**
  2026-09-12（A-17 修正）：用 `git checkout -- <file>` 还原变异，**把尚未提交的修正一并还原掉了**，
  而下一个变异是在少了修正的树上跑的。**"还原到 HEAD"只在被测改动已经提交时等于"还原变异"。**
  **做法**：先提交再变异；或备份文件、变异、从备份拷回，并在前后各核一次被测改动仍在。
