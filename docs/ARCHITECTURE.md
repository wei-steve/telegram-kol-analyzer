...
- **一个窗口的证据价值由被改的判据决定，不由窗口的健康度决定。** 30 分钟零重置、全绿、
  ...
  **真空正是它唯一的实测条件**；B 线 6a 改的是保护单归属与替换顺序，**没有仓位就没有可归属的对象**，
  ...
  守卫退化根本没走到判定）。
  **写下判据只是第一步，第二步是拿判据逐条去对采样行的字段名**——确认每一条判据都有一个字段
  会真的记下它。2026-09-10 6b 起窗一分钟后发现：判据要的 `cancel_precheck` / `ledger_drift`
  在观测脚本里**根本不被采集**，采样行里没有这两个键。**缺字段的采样表会以"合格"的样子交付
  一个填不出来的判据栏**，而且它比一个恒为 0 的计数更隐蔽：恒为 0 至少还在行里、是个能被看见的
  异常，字段不存在的采样行**看起来完全正常**，只有拿判据逐条对照才发现对不上。
