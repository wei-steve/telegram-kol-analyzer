...
position_protection_notification_status: blocked_historical_backlog_gate_missing
...
server_verification:
  ...
  position_protection_notification_blocker: "The approved deterministic severe-protection notification activation was refused before any configuration change. Production contains 256 generation-1 severe_protection_incident runtime rows with notification_status=pending and no active claim. The notification worker claims the oldest pending row solely by exact incident type and has no activation-time or minimum-generation cutoff. Adding severe_protection_incident to TELEGRAM_KOL_RUNTIME_INCIDENT_TELEGRAM_TYPES would therefore deliver the historical backlog in batches of up to 20 rather than one new-generation canary. Existing Telegram and Agent selectors remain exactly management_partial_failed; no incident was claimed, delivered, suppressed, or rewritten, and no service restart occurred. Safe continuation requires a reviewed dormant notification-not-before eligibility gate, deployed with selectors unchanged, followed by a separately approved activation for incidents first occurring after the recorded cutoff. Do not bulk-mark the 256 historical rows delivered or not-needed."
