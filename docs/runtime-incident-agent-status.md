...
message_operation_extension:
  ...
  allowed_symbol_contract_spec_status: dormant_static_deployed_shadow_blocked
  ...
  allowed_symbol_contract_spec_activation_blocker: "Do not enable shadow or live mode while the independent no-notify diagnostic is unhealthy with contract_violation_missing_stage1 and message_operation_supervisor_stale. Read-only evidence shows the nine affected message-operation contracts/items and supervisor failures predate the dynamic-spec deployment; this deployment created none of them. Resolve that separate production incident and repeat the zero-in-flight, protection, monitor, and cache gates before any activation."
  system_bot_credential_incident: "A main-service shutdown exception journal entry exposed a system-bot credential inside a request URL. The value is intentionally not recorded here. Treat it as exposed and rotate it through a separately authorized incident response before relying on the credential again."
