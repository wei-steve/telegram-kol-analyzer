...
production_commit: 04b3d3445b37f4da36ccf12a6c199e012b7ede65
local_tests:
  - "phase-8r.7-local-initial: 184 focused security, broker, tool, service, schema/migration, exchange, Telegram, production-audit, worker, incident, and ownership regressions pass. The audited declarative broker has nine broad read-only categories, exact incident binding, closed query names, bounded IDs/time/bytes, durable started-plus-terminal audit metadata, opaque-credential/JWT/evidence-reference rejection, pre-read SQLite/file byte limits with live-WAL visibility, read-only file/network adapters, no Agent default grant, bounded WAL-recreation access with non-enumerable sticky data-directory DAC, explicit existing non-database ACL and group/other mode denial, restrictive future inherited ACLs, session mount denial, and an exact no-follow/open-descriptor SQLite ACL pre-start restoration helper, bounded evidence adapters, and a private sidecar workspace. It remains unregistered from Agent tools and selectors. Independent re-review of the final sidecar hardening remains pending."
...
server_verification:
  ...
  remaining: "Phase 8R.7 local implementation is in progress. Complete focused and adjacent regressions, independent review, push, and a fresh safe-window dormant server deployment. Server verification must install the hardened sidecar only while stopped/disabled, run isolated read/mutation-refusal canaries without changing Agent eligibility, and prove service/listener/scanner/monitor continuity. Completion also requires a trade-disabled Deepcoin evidence credential or equivalent trusted read-only loopback projection and deployed egress-enforcement evidence; the current application host policy alone is not treated as exchange-credential or OS-network proof. Do not begin 8R.8. Natural 8R.6A/8R.6B behavior and all-symbol contract specs remain separate residual scopes."
enabled_flags:
...
- Next action: finish independent review and dormant server verification. Keep
  8R.7 in progress until a trade-disabled exchange evidence credential (or
