...
server_verification:
  ...
  remaining: "Phase 8R.3 stays in progress only for observation of the first future natural multi-target failure notification. Deterministic notification is active now and uses the unified AI agent notification title. After one real target incident proves capture and delivery end to end, a later single stage may widen the read-only Runtime Agent selector to the five exact target incident types. The four new scanner compliance rules remain non-deployable and disabled; no trading or playbook authority is granted."
enabled_flags:
