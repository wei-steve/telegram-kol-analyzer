...
server_verification:
  ...
  remaining: "Phase 8R.2 remains in progress. Do not widen production capture yet. The monitor-owned adapter cannot append runtime incident rows through its intentionally read-only production database mount; granting whole-SQLite write access was reviewed and rejected because it would also authorize business-table mutation and has unsafe WAL lifecycle semantics. Implement a bounded loopback writer in the trusted main service or a dedicated incident outbox/importer, then repeat review, safe-window deployment, and one-type-at-a-time capture comparison with Telegram and Agent selectors unchanged. Do not implement the invariant scanner or any later Phase 8R stage."
enabled_flags:
