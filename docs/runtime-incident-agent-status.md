...
position_protection_notification_status: watermark_pushed_deployment_deferred_safe_window
position_protection_notification_selector: management_partial_failed_unchanged
context_target_contract_repair_status: pushed_deployment_deferred_safe_window
...
server_verification:
  ...
  position_protection_notification_watermark_predeployment: "Reviewed watermark commits through 1678239 were pushed to codex/deepcoin-auto-trading-v1. Production remains on f26e71b with both selectors exactly management_partial_failed and TELEGRAM_KOL_RUNTIME_INCIDENT_TELEGRAM_AFTER_ID absent. Deployment was deferred: while the zero-in-flight checks were being repeated, raw messages 9756-9758 arrived; 9758 eventually reached a terminal completed decision with authoritative_status recognition-failed. The gate therefore did not claim a healthy latest recognition or a stable two-pass quiet window. One complete four-position exchange snapshot matched fingerprint ba96f20531a88016cc10e7b59c3e4c28b7a3ab7a88932c3e656a8fb3b57d9b90, but later bounded reads timed out, so a fresh repeated complete snapshot was also not proven. Evidence/context/management/component/position-mutation/recent-execution/runtime-Agent/runtime-notification/recovery in-flight counts were zero at the last completed pass; all 256 historical severe-protection notifications remained pending and unclaimed. Required continuation: wait for a later natural message to complete recognition successfully, then re-run two stable zero-in-flight and complete exchange-snapshot passes before restarting."
  ...
  context_target_contract_predeployment: "Reviewed context target-contract repair commits through 33f921a were pushed to codex/deepcoin-auto-trading-v1. Production remains on f26e71b. The deployment gate stopped before restart because no natural message arrived after raw 9758; the latest authoritative decision is still raw 9758 with authoritative_status recognition-failed, comparison_status completed, and automation_status skipped. Evidence extraction claims are zero, but the required healthy latest natural recognition cannot be proven. No server code, configuration, service, database row, notification selector, runtime watermark, historical recognition, or exchange state was changed. Required continuation: wait for a later natural message to complete authoritative recognition successfully, then run two stable zero-in-flight and complete exchange-snapshot passes before deployment. Never replay raw message 9758 to create that proof."
