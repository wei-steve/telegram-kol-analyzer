...
production_commit: f95cbcfe2ff6e20bca9d40246d9dcedf06f5d44a
local_tests:
...
server_verification:
  ...
  remaining: "Phase 8R.5 code is reviewed and deployed dormant at f95cbcf, but its future-only shadow canary is blocked by assembly 3's exact terminal pre-binding SOL sizing refusal being classified as live_entry_preamble_binding_evidence_missing. Keep the supervisor disabled and both contract tables empty. A separate approved design is required before changing monitor classification or SOL trading configuration; only after a healthy diagnostic and fresh zero-in-flight gate may the 8R.5 canary run. Do not implement outcome supervision, Stage 1 notification, broad Agent eligibility, or business mutation while 8R.5 remains in progress."
enabled_flags:
...
- Task 8R.5: reviewed deterministic zero-model projection is deployed dormant
  at `f95cbcf`; activation keys remain absent and both additive tables remain
  empty because the post-deployment independent diagnostic detected
  `live_entry_preamble_binding_evidence_missing`.
- Next action: obtain approval for the narrow monitor-classification design
  that recognizes exact terminal pre-binding no-submit evidence without
  changing trading or rewriting assembly 3. Keep SOL contract-spec and
  allowed-symbol consistency as a separate trading-affecting decision. After
  the reviewed monitor fix produces a healthy diagnostic, prove a new
  zero-in-flight gate, record the current maximum raw-message ID, and run
  exactly one future-only `--shadow --once` canary. Do not start outcome
