...
local_tests:
  ...
  - "phase-8r.6b-initial-review-fixes: two Important findings were reproduced and fixed under TDD: eligibility now uses the per-message contract watermark so an old coalesced RuntimeIncident cannot suppress a new source-message alert, and Stage 1 delivery failure creates a separate high-severity notification_delivery_failure incident without persisting exception text. Independent re-review remains pending."
