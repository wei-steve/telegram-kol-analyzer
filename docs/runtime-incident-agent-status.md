...
server_verification:
  ...
  remaining: "Phase 8R.5 code is reviewed and deployed dormant at f95cbcf, but its future-only shadow canary is blocked by the independent live_entry_preamble_binding_evidence_missing safety invariant. Keep the supervisor disabled and both contract tables empty until a fresh healthy monitor diagnostic and zero-in-flight gate are proven. Do not implement outcome supervision, Stage 1 notification, broad Agent eligibility, or business mutation while 8R.5 remains in progress."
enabled_flags:
...
- Next action: resume only 8R.5 after a fresh healthy monitor diagnostic and
  zero-in-flight gate, record the then-current maximum raw-message ID, and run
