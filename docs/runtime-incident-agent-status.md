...
current_phase: 8R.7
phase_name: broad-enforced-read-only-investigation-broker
...
last_completed_phase: "8R.6B"
last_completed_commit: 04b3d34
production_commit: 389b6331688eede5a8b896c10efb924f9e812f9a
local_tests:
  - "phase-8r.7-egress-local: fixed MiMo Unix-socket relay and Runtime Agent cgroup IP deny are implemented under TDD. The Agent LLM client accepts only /run/telegram-kol-agent-model-egress.sock, the relay has one immutable api.xiaomimimo.com:443 upstream, and direct public IP egress is denied while loopback evidence remains available. Deployment and independent review remain pending; the broker is still unregistered."
...
server_verification:
  ...
  remaining: "Phase 8R.7 code review and dormant server deployment are complete. Completion still requires a trade-disabled Deepcoin evidence credential or equivalent trusted read-only loopback projection and deployed OS egress-enforcement evidence; the current application host policy alone is not treated as exchange-credential or OS-network proof. Keep the broker unregistered and do not begin 8R.8. Natural 8R.6A/8R.6B behavior and all-symbol contract specs remain separate residual scopes."
enabled_flags:
...
message_operation_extension:
  ...
  status: phase_8r_7_in_progress_local_broker_dormant
  ...
  task_8r_7_status: in_progress_local_review_and_server_controls_pending
...
- Current task: `8R.7 broad-enforced-read-only-investigation-broker` (in
  progress; local broker implementation is dormant and server isolation gates
  remain)
...
- Task 8R.7 local implementation: nine closed evidence categories, query-only
  ...
  existing Agent tool registry and selector remain unchanged.
- Next action: provision and verify the remaining external controls. Keep 8R.7
  in progress until a trade-disabled exchange evidence credential (or
  equivalent trusted read-only loopback) and deployed egress enforcement are
  proven. Do not begin 8R.8, contract-spec expansion, or business mutation.
