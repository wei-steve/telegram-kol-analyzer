...
- step-11b (2026-09-10, local_22ee72a5-d88c-4ba2-9b17-366585562d10): **A-11 那张 AST 清单上的五处，逐处通读所在流程后分类**（L2，分支 `mgmt/step-11b-same-class-sites`）。裁定要求：能证明提交前抛出的归确定性失败侧并告警，有"可能已写"歧义的保持 `recovery_required` 并写明理由，不确定就留在 `recovery_required`。**结论是 2 处改、3 处刻意不改，而分界线不是异常类型，是"这次确定性失败留下了什么"。**
  ...
  **新增 5 条用例**（每处一条）：refused 的保护替换走回滚而非停等（并断言 `recovery_required` 不出现）；该拒绝的 incident 行**真的存在**且带 reason（A-10e 的教训）；回滚中的 refusal **仍是** `recovery_required`；回执缺失**仍在**未知侧；restore 的 refusal **仍是** `recovery_required`（并直接对源码断言调用方那条降级契约，而不是对一个替身断言）。**三条"仍是"的用例是刻意写的**——原意是让下一个人在没读流程的情况下改不动这三处。**（A-15 更正：这三条经变异检验**不能**担保这一点，见 step-15。）**
...
- step-15 (2026-09-10, local_22ee72a5-d88c-4ba2-9b17-366585562d10): **"必须同时有 TP 与 SL"专项扫描 + 修那道过不去的闸门**（L2，分支 `mgmt/step-15-both-triggers-required`）。动机见 step-14：一天里撞到四个同形状的判据，怀疑是阶段 5 把入场从"带一对"改成"只带止损"时下游一批判据没跟着改。
  **(1) 专项扫描：假设被显著缩小，如实记。** AST 找出全部 `and` 合取里同时提到 tp 与 sl 的共 **39** 处，逐处读源码后**只有两处真正"要求两者都存在"**，第三处待 B 线确认：
  | 位置 | 判定 | 生产可达性 / 后果 |
  |---|---|---|
  | `execution_bindings._request_has_combined_trigger_protection` **2942** | **命中** | A-12 已查明：挡住"入场自带止损"的采纳，7-24 后零产出；归 6e/6h（B 线） |
  | `position_attribution._reviewed_position_matches_live` **638** | **命中** | 本步修（见 (2)） |
  | `recovery_live_submit` **2907** | **待确认** | 要求两者都在，但它匹配的是一个**期望为组合单**的行（`expected_by_purpose` 同时索引两个键），**多半正确**；B 线的文件，请它核调用方 |
  其余 36 处**不是**这个形状：`protection_attribution` 60/132、`strategy_management_executor` 3735 都是"两者都有 → combined，否则走单边分支"的**分类**；`message_recognition` 3719/3747/5055 是"**两者都没有**"的反向判定；`position_attribution` 1145/1188 是数值比对（`_number_tuples_equivalent((), ())` 为 True，空对空等价）。
  **所以 step-14 那个"下游一批判据没跟着改"的假设，被这次扫描缩小成"两处（另一处待确认）"。** 我把假设与它的缩小都留在记录里——**假设本身是有用的（它换来了这次扫描），但它不是结论**。
  **(2) 修 `_reviewed_position_matches_live`：整个修法是删掉两个合取词**。原代码要求 `reviewed.take_profits and current.take_profits` **两侧都非空**；而**下面那行比较本来就说了该说的话**：`_number_tuples_equivalent((), ())` 为 `True`（都没有 = 等价）、`(1.0,)` 对 `()` 为 `False`（一侧有一侧无 = 已变）。**要求两者非空说的是另一件事，且从来不是本意。** 三条用例锁住：只带止损的仓位**与自己比为 `True`**（修前为 `False`）、止盈从有到无与从无到有均为 `False`、止损比对与止盈价变化仍为 `False`（未动）。
  **(3) 只读追这道闸门的先后——结论更正了 step-14 的措辞。** 我在 step-14 写"它由 `strategy_management_worker`（两处）与 `deepcoin_execution_actions` 调用，**是管理执行路上的闸门**"。**这句太松。** 逐处追调用方：
  · `strategy_management_worker` 两处在 **`_resolve_deferred_entry_cancel_race_successor`** 与 **`_resolve_capability_deferred_successor`** 内——**是竞态/后继解析路径，不是主执行路径**；
  · `deepcoin_execution_actions._require_live_position_economics` 被 **`adjust_position_tpsl`**（人工 TPSL 调整；自动来源在更早处被 `automated_position_tpsl_requires_management_batch` 挡住）与 **`close_bound_position_market`**（两处）调用；
  · **普通管理批次（`execute_management_batch`）根本不经过这道闸门**——它撞的是保护替换预检 `protection_preflight_rows_ambiguous_or_drifted`（step-12 更正后那条）。
  **所以修前在只带止损的仓位上：人工 TPSL 调整、市价平掉在管仓位、两条后继解析——四条路被这道闸门拒；而普通管理批次被另一道（保护预检）拒。**
  **(4) 必须写明：本步不是无操作，它重新放开了正在拒绝的路径。** 尤其 **`close_bound_position_market`**——修前它对只带止损的仓位会拒，**修后可以通过**。那是一条**真的会平掉真实仓位**的路径。本步没有改它任何其它判据（止损比对、尺寸、保证金模式、instrument、方向全部不变），放开的只是"没有止盈不再被当成经济学已变"，**而那本来就是本意**；但"从拒绝变为允许"这件事本身值得部署前被看见，不该藏在一句"修了一个闸门"里。
  **(5) A-11b 三条用例的变异检验：结论是我的注释担保不了它宣称的东西。** 按 B 线的标准（要证的是"删掉那三处分类逻辑它们会红"），我把三处**反向变异**（回滚撤单的 refusal 视为成功、restore 助手对 refusal 报成功）后重跑——**三条全绿**。逐条原因：
  · 回滚那条断言写成 `"recovery_required" in [各腿状态]`，**兄弟腿因自身原因也是 `recovery_required`**，所以 `in` 恒真；我把它收紧到具体那条腿之后**再变异仍然全绿**，说明那条腿是因别的原因落到该状态——**我没有追到底**；
  · 回执缺失那条断言的是**批次级** `result["status"]`，与回滚变异无关；
  · restore 那条断言的是**调用方契约的源码文本**（`inspect.getsource`），**根本没有执行到助手**，所以改助手它不会红。
  **处置照裁定：把注释改成它实际能担保的话**——三条现在写明"**记录当前行为，而不是防卫它**"，并在 step-11b 相应那句加了指向本步的更正。**收紧那条断言我保留了**（断具体腿比断列表更准），但注释里写明**变异后它依然全绿**，免得下一个人再把它读成保证。
  **(6) 补 step-14 一条证据（按 B 线建议：引证据而不是引结论）**：step-14 记的是"A-14 部署零交易所写入（B 线测得）"，那是**结论**。证据是——`position_mutation_intents` 全库在 A-14 部署前后**只有 `id=664` 一行**；B 线第二笔备份止损之后是 **664、665 两行**。**理由**：结论过期不会报错，数字过期会对不上；下一个人能拿数字去撞，拿不了结论。
  **(7) 全量对账，差 1 也停了一次**：预期 `8340 + 3 = 8343`，实际 **8344**。查出差的那 1 条是 B 线合并提交里的一条测试参数化（它此前在消息里提过"6f 的一个常量、一条测试参数化和文档"）。**对账完毕**。记在这里是因为本步刚把"矛盾的数字是待办"写进 `ARCHITECTURE`——**这一条就是它的第一个正面用例，而差 1 与差 100 一样值得停一次**。
  **(8) 部署前的一次假阴性，差点让我上进别人的窗口——而它是我自己那条"锚定完整命令行"的建议造成的。** 我用 `ps -eo pid,etime,cmd | awk "/^ *[0-9]+ +[0-9:-]+ +bash \/root\//"` 数 B 线的监视器，得到 **0**，正要按"它已收窗"部署。**实际它在跑**（PID 18182）——命令行是 `/bin/bash /root/step6f2_observe_v2.sh …`，而我的模式要求 `bash /root/`，被 `/bin/` 前缀挡掉了。
...
- **（从作废提交 `16b578de` 中救回的文档部分——该提交的代码部分是已作废的第二仓位放开，**不合并**；只保留这段证据，因为它只存在于那一个从未推送的本地提交里）**
  **把检查写成会自己判 PASS/FAIL 的形式**（指挥会话建议，采纳）——**但它推荐的那个写法在这台机器上是坏的，我用它的第一次就被它骗了，照实记。**
  建议的写法是 `git diff <prod> <candidate> --name-only | grep -qvE '^docs/|\.md$'` 命中即 FAIL。**实测它恒判 PASS。** 最小复现：
  ```
  printf 'a.md\nb.py\n' | grep -vE '^docs/|\.md$'        # 打印 b.py
  printf 'a.md\nb.py\n' | grep -vE '^docs/|\.md$' >/dev/null; echo $?   # 退出码 1
  type grep  → shell function → ugrep 7.8.4
  ```
  **这台机器上的 `grep` 是 `ugrep`，`-v` 的退出码语义与 GNU/BSD 不同**：有反选行时仍返回 1。于是
  `if ... grep -qv ...; then FAIL; else PASS; fi` **永远走 PASS 分支**。我用它判"生产到现在代码变了没有"，
  它答 PASS，而同一条命令把 5 个 `.py` 文件名打印在下一行——**判据与证据当场自相矛盾，我因为先看见 PASS 差点读过去。**
  **改成计数式**：`N=$(git diff A B --name-only | grep -vE '^docs/|\.md$' | wc -l)`，`N -eq 0` 才是 PASS。
  **并且回头用计数式重做了今天全部四次"仅文档"判定**：首笔部署时 0、首笔后推文档 0、影子部署时 0、当前生产 vs 共享分支 0——**四次结论都对**。它们没被这个坑害到，是因为那几次我**把过滤结果打印出来**、没有只依赖退出码；**唯一被骗的一次，正是我改用"更严谨的"纯退出码判据那一次。**
  **这条比前三次都难堪，因为它就发生在我把"判据要自己说话"写进文档的同一屏之内。** 教训要再收一格：**判据自己说话还不够，判据本身也要被验证过**——`grep -q`、`[ -n ]`、`test` 这类退出码判据，**在依赖它做不可逆决定之前，先用一组已知真假的输入喂它一次**。
...
- **A-15 + A-16e + A-17 合并部署，以及它在第一轮就违反了"只向前"**（2026-09-12）。
  **部署**：候选 `90403489342eaf5493cc775c33ad43378c1998b0`，**回滚参考 `1fd45bc266ec3217c9e8b687482ded1b9f4fa8cd`**，21:00:30Z 部署，共享分支已推同一 sha。全量 **8528 passed / 4 skipped**，逐文件对账 8470 + A-17 17 + A-15 3 + B 线 6k 38 = 8528（第一次对账脚本在 zsh 下不拆词、只循环一次并给出"净增 0"，作废后用 bash 加哨兵重做）。部署后正向核对：A-17 调用 11 处、TP 守卫 1、A-16e `first_pass` 写入行在、A-15 新用例在且旧合取词两行为 0。
  **事故**：L1 观察器开窗时快照打印 `retired_at_start=664`，而部署前只读试跑是 0。只读核对：**部署后第一轮对账（21:00:50Z）在 83 个早已关闭的 binding 下把 664 行置 `retired`**（账本 208 行 / 56 个 binding，腿 456 行 / 73 个 binding，原状态 planned 248、verified 173、protection_recovery_pending 25、waiting_fill 10），全部 `retired_by=entry_legs_terminal`；第二轮起不再增长；活仓 binding 352 未受影响；已关闭 binding 下仍未退役 808 行。
  **原因**：`_apply_reconcile_snapshot` 每轮加载 `closed`/`cancelled` 在内的 binding，逐个过 `_derive_binding_from_entry_legs`；entry 腿全终态时对已关闭 binding 再写一次 `closed`，而终态化是无条件调的。修复函数的 terminal 分支是同一形状。
