...
## 10. 更正第 8、9 节：改造**已经**解决了这件事，用的正是 WS

**第 8 节说「未成交入场单自带的保护单没有归属载体、是结构性缺口」，第 9 节据此
说「WS 给 id 但不给关联」。两条都错，在此收回。** 错因是查错了列：
`deepcoin_ws_events.position_id` 只承接 `Position` 帧的 `PI`，而
`TriggerOrder` 帧的归属证据在 **`TU`**。按 `TU` 重查：
...
**而它的 order id 从不出现在下单回执里**——正是用户凭记忆指出的那条 REST 结论。
既有两条入场保护记账路径（`entry_protection_response` 读回执、
`execution_bindings` 的采纳路径要求 `order_kind == "trigger_limit"` 且请求同时带
`tpTriggerPx` 与 `slTriggerPx`）都会漏掉它。2026-09-10 的生产后果：
两个活仓各自在交易所上有止损、账本一行都没有、旧匹配器报 `absent`、
`backup_stop_blocked` 被记了两次。

6e 的解法不是放宽旧闸，而是并行开一条以阶段 6 证据为准的新路：
...
采纳的行标 `evidence_source = "exchange_adopted_by_tu"`，且**只写账本、不发交易所
请求**，唯一可能把账本行变成交易所写入的消费者（`trigger_backup_stop_executor`）
要等 6f 单独批准。

### 链条是闭环的（只读核对，2026-09-23）
...
`exchange_adopted_by_tu` 已产出 11 行。

### 所以用户此刻看到的「未归属」是设计内的正常状态

那四张自带止损的 `TU` 现在都是 `default`——**入场还没成交，此刻根本不存在可归属的
仓位**。等入场成交，交易所把 `TU` 改成 posId 重推一次，6e 采纳并记账，归属才出现。
在那之前显示「未归属 · 自动管理已冻结」是对的。

### 剩下的真缺口：7 条漏网

| 帧时间 | OS | TU | TU=OS+1 |
|---|---|---|---|
| 09-07 07:36 | …167675480 | …167675481 | 是 |
| 09-07 12:55 | …170888257 | …170888258 | 是 |
| 09-07 13:22 | …171164581 | …171164582 | 是 |
| 09-09 15:25 | …202354519 | …195880289 | 否 |
| 09-09 19:50 | …194996696 | …194996697 | 是 |
| **09-15 06:53** | …279571459 | …277441648 | 否 |
| **09-21 20:26** | …373782707 | …370127138 | 否 |

前五条都在阶段 6 收口（2026-09-12，生产 `1fd45bc2`）**之前**，是历史遗留。
**后两条在其之后**，且 `TU` 与 `OS` 不相邻（不是入场自带形态），
`TU=posId` 的帧**各只出现过一次**。这正对上 `README` §7 第 2 点列为
「三个最不确定技术点」之一的风险：错过那一次 `default → posId` 推送就没有第二次，
而 6e 的采纳还要求该 ordId 在核验时仍在挂单表里——成交或撤销得够快就采纳不上。

**这两条值得单独查**，它们是「证据到过、我们没接住」，与本文前九节修的展示层 bug
无关，也与入场未成交的正常状态无关。
