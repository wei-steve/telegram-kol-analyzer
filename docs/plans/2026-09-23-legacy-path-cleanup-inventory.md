...
## 7. 第 6 节的缺口已修并上线（2026-09-23）

**生产 sha `77df50d406f5e029bec853fafda3e2bf251e0c04`**，回滚参考
`75da0b4ef2fc3890e9ea565ef52a67c7724cdf7d`。同一批还带上了第 4.2 节的删除。
四步流程走完，双向核对 PASS（`PROD == SHARED`、`OFFENDERS` 空）。

用户指出「排除」这条规则的出身：REST-only 时期，随委托单一起挂上的止损查不清归属，
又不敢不带止损，于是先带上、成交后再撤掉重设，查不清的那一半只能靠排除兜底。
**REST+WS 之后这个前提不成立**——`TU` 是交易所直说仓位存不存在，四元组匹配的是
我们自己的持久腿记录。判据本身够用，缺的只是把腿留下：
`_pending_entry_stop_signatures` 丢掉了腿（排除只需要知道「是不是我们的」），
新增的 `resting_entry_stop_owners()` 保留它，页面读这一份。

**上线后实读（生产页面，非本地渲染）**：

```
挂单入场自带止损（入场尚未成交）
  BTC 空 88300  5 张    order …386088032  比特币陈哥会员群-11分组 · BTC short · entry leg #2
  ETH 多 2700   1.6 张  order …385572806  大镖客 11分组         · ETH long  · entry leg #2
  ETH 空 3100   0.3 张  order …381417341  大漂亮社区 11分组      · ETH short · entry leg #1
  ETH 空 3100   0.5 张  order …381417387  大漂亮社区 11分组      · ETH short · entry leg #2

未归属交易所保护单 —— 该区块未渲染（一张都没有）
```

**四张全部正面归属到群与腿，「未归属」归零。** 同群的 leg #1 / #2 只靠 `sz`
（0.3 / 0.5）就分开了。三个活仓、四张挂单，页面与委托分页均 200，窗口内
web 与 worker 日志 0 条 traceback / CRITICAL。

权威链行为未变：这四张仍被 `resolve_protection_authority` 排除，不进保护集合、
不冻结、不撤。

## 8. 仍未处理，需要单独批准

用户同时指出：既然归属已经查得清，「先附带止损 → 成交后撤销重设两重止损」这套
**交易流程**本身也可以简化。这改的是交易所写入语义（L3，`AGENTS.md` 要求单独批准），
且动手前要先查清一件事：成交后重设的那「两重」里，备份止损
（`trigger_backup_stop_executor` 那条线）可能有独立于归属的风控理由，
不一定能跟着一起简化。**先只读查清这条线，再谈改不改。**
