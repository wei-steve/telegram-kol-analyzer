...
- 真实风险信号：2026-09-06 17:04:07Z 生产的 `GET /deepcoin/trade/trigger-orders-pending`
  出现过 401 Unauthorized，阶段 2 的 5 小时开放式观察里再次复现，累计两次。
  尚不能判断是签名时间戳容差还是限流，所以本阶段先记录归因所需字段，不做修复。
  唤醒触发的 REST 核验遇到这类偶发
  失败必须记为 unknown / incomplete 并让下一次轮询重试，绝不能降级成"零"或"无"，
  也不能因为一次 401 就把连接状态机推到 `disconnected`（那是 REST 的事，不是 WS 的事）。
  为它加一个离线测试。
- 归因能力（阶段 2 开放式观察遗留）：31 次静默重连里 4 次（约 13%）首轮重同步出现
  `incomplete_rest_read`，都在数秒内重试收敛，但结果只记了结论没记细节，无法判断是否与
  间歇 401 同源。本阶段把重同步与唤醒核验里每次失败的具体调用名、HTTP 状态码、异常类型
  记进结果与健康端点（不记响应体、不记凭据），让 incomplete 可归因。
## 禁止
...
- 禁止对任何不属于 `execution_bindings` 的订单、条件单或仓位做写入（撤单、改单、认领）。
  阶段 0 的三张历史 pending 条件单用户已于 2026-09-06 自行撤掉，本条对之后出现的
  任何手工/遗留单同样成立。
...
- [ ] **第 7 项：手工订单与系统订单并存。**
      ...
      - 生产：按任务 5 执行。观察前只读列出交易所上所有不属于任何 `execution_bindings`
        的订单、条件单与仓位；若存在，观察窗口前后各取一次完整原始行，逐字段比对必须
        相同且零写入；若列表为空，记录"第 7 项本阶段只有离线证据"，留给阶段 5 的受控
        实验。不要为了制造基线去下手工单。
...
- [ ] 观察目标：一个连续 30 分钟窗口，含至少 5 条真实消息、尽量 2 个群。
      消息流量不受任何人控制，安静窗口不算失败：按 `AGENTS.md` 的 L2 规则在服务器上
      起只读后台监视器，每分钟采样、凑够合格窗口自己停，会话用定时器查结果而不是坐等
      轮询（阶段 2 的
      `/var/lib/telegram-kol-cutover-evidence/rest-ws-phase-2/open-observer.sh`
      可复用，补上本阶段新增字段）。等待上限 24 小时；若仍无流量，记录并把阶段留
      `in_progress`。
...
- [ ] 直接查交易所历史：本阶段窗口内的所有交易所写入都必须能由生产正常交易解释，
      条数与 binding 数对得上。
- [ ] 任务 5 列出的非 binding 订单/条件单/仓位前后完全一致；列表为空则记录
      "本阶段第 7 项只有离线证据"。
- [ ] 因为本路径可影响执行，检查 `authoritative_execution_attempts`
      在窗口内没有新增 `uncertain`；若有，必须能归因到正常交易而非本改动。
- [ ] **必须重启 worker 一次。** 本阶段确实触及进程生命周期：任务 1 的唤醒原语与
      等待任务要随 worker 任务重建，任务 6 的乱序 tracker 种入要在重启后由重同步第 4 步
      重新完成。重启后确认唤醒计数恢复增长、`tracked_entity_count` 不再为 0、
      定时轮询照常。重启前先确认没有正在执行的管理批次
      （`active_write_count=0`，无 planned/executing/reconciling 批次）。
...
等价性测试结论：
补测项 7：离线结论 + 非 binding 订单前后比对结论（或"只有离线证据"）
测试：focused N passed；全量 N passed / N skipped / N failed（列出既有失败）
观察窗口：<起> ~ <止>（30 分钟合格窗口），等待总时长、真实消息数、覆盖群数
重启：<时间>，重启后唤醒与 tracker 状态
incomplete REST 读归因：调用名 / HTTP 状态码 / 异常类型 / 次数
