...
交易所上现有三张历史 pending 条件入场单（见
`docs/rest-ws-trading-status.md` 阶段 0 结论第 2 条）：
ETH `1001125109770664`、ETH `1001125109770668`、BTC `1001125122023458`。
它们不属于任何 binding，正好是"手工订单与系统订单并存"的真实基线。

本阶段要证明：唤醒式 reconcile **不会**改变系统对这三张单的处理。
观察前后各取一次它们的状态，必须完全一致，且系统不得对它们发起任何写入。

**它们当前正在全局否决止盈收敛，这是既有缺陷，本阶段不修、不撤、不改。**
若观察中发现系统开始对它们做任何动作，立刻停止并报告——那说明唤醒改变了行为，
违反本阶段的"只改什么时候跑"约束。
