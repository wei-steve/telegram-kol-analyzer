...
| 7 | `phase-7-retire-legacy-matcher.md` | 退役推断式匹配器 `match_position_protection` | **L3** | 删掉回退实现，等于把新链的每个缺陷从"两条链不一致"降级成"无人反对" |

**阶段 6 已于 2026-09-12 收口**（`phase_status: completed`）。
它遗留的**不是未完成的工作，是未到达的样本**：6b 切换、6g 切换、登记簿 P1–P7。
这些由巡检与登记簿跟踪，不阻塞收口——**但它们是阶段 7 的前置**，
所以阶段 7 现在是 `planned` 且**不可开始**。

**注意编号撞车**：交接文档里也有一个"第 7 步"（真正的条件触发策略继续用 trigger-order
并保留独立父子归属流程），**那一条不是上表的阶段 7**——它是**保持现状**，
写进阶段 5 的"禁止"里：阶段 5 不得把真正带突破/回落触发条件的策略腿改成普通 order。
