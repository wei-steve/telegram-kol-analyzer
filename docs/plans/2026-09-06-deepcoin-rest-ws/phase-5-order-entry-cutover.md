...
## 批准时补充的硬性约束（2026-09-07）

1. **`place_order` 的响应体必须整体持久化。** 阶段 4 实测：REST 没有任何读接口能把 ordId
   关联到 posId，posId 只在 `POST /deepcoin/trade/order` 响应体顶层出现一次。任何"只存 ordId、
   以后再查"的设计都会永久丢失确定性链接。现有 `direct_order_position_id`（tier 0）已在用，
   新路径必须沿用并把整个响应 JSON 原样落库。
2. **"缺口期间暂停新入场"不得放松。** 阶段 4 补测第 12 项结论：Deepcoin 私有流重连不重推，
   只推变化。错过 `TU: default → posId` 那一帧就没有第二次机会，REST 侧无法佐证。
3. **收益定位。** 阶段 4 的 `timing_only` 中位提前量为 -2.652 秒：市价入场的 posId 在下单响应里
   同步可得，WS 在入场归属上没有延迟收益。本阶段的价值是**确定性**：消除按价格、数量、时间
   做的推断式候选匹配。汇报里不要用"更快"作为结论。
4. **实盘实验的执行方式。** 执行会话不得自行提交任何真实订单。它负责：准备每一笔实验的精确命令
   与参数、在下单前建立 WS 订阅并确认健康、由用户本人在终端执行下单命令、随后采集回执与 WS
   事件并写证据。每一笔都要先向用户出示命令再等用户执行。
5. **与事故修复的关系。** 2026-09-07 事故排查（`docs/2026-09-07-management-instruction-incident-read-only-diagnosis.md`）
   证实"被推迟的指令永不恢复"与"入场失败的策略被模拟成已入场"两个管道缺陷，它们在
   `docs/management-reliability-status.md` 的 A 线修复；本阶段不修它们，但验收时若新入场被推迟
   或静默丢失，必须如实记录为 A 线未完成导致，而不是归因于本阶段。
