...
这是 additive schema 变更，必须先在生产库副本演练，再作为独立 L3 schema 动作安装。旧 runtime
完全忽略该表。runtime 激活前必须显式验证表、约束和索引存在，不能依赖 activation 时的
...
- SIGTERM：worker 先停止 claim 新 message job，等待
  `MessageProcessingActivity.active=0` 与本 invocation 的 active attempts 清零，再结束进程。当前
  `web_app.py:5159` 直接 cancel 的行为改为 bounded drain；因为 cancel `to_thread()` 不会停止
  底层线程。
...
- 只新增 `authoritative_execution_attempts` 表/索引/约束，不改现有表与业务数据。
...
- 回滚：在任何新 runtime 使用该表前可直接 drop；一旦已有 attempt，先导出并校验，再先回滚
  runtime、后 drop 表，避免 `create_all()` 重建。
...
- 初次激活时 scanner 只观测新 attempt，不自动接管 legacy 29 行。
...
- SIGTERM 正常窗口完成 drain；SIGKILL 模拟只产生 safe pre-boundary failure 或 post-boundary
  uncertain，不产生第二次 adapter 调用。
- `execution_uncertain` 永久阻止新 authoritative overwrite 与 backlog expiry，直到独立
  reconciliation。
- 自动检测能在一个扫描周期内报告“job succeeded + decision running”。
