...
phase_5_rollout_status: "dormant code deployed and the extracted shadow path processed one real post-deploy message correctly; production cutover was then BLOCKED before any queue flip because the quiet-window check found an actively updated reconciling management batch"
phase_5_outstanding: "Wait for a genuinely quiet window with zero in-flight management batch (do not infer safety only from active_write_count=0); prove shadow -> queue -> shadow with exactly one processing outcome at the transition boundary; re-check the safe window; enable queue; observe one complete real trading session; deliberately restart mid-traffic with a durable pending/claimed job and prove resume; verify missing/orphan/stuck/backlog plus loop lag; compare execution_events and Deepcoin regular/trigger order history directly for duplicate orders; only then complete Phase 5 and point to Phase 6."
