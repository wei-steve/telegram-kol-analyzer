...
phase_6a_outstanding: "Obtain explicit owner approval to treat the currently disabled notification bot as the safe freeze gate for one real sync shadow parity sample, with a fresh pre-call recheck that the current service still has notification_bot_effectively_enabled=false and a post-call proof that no pending/delivering/delivered notification state changed because of the sample. Do not send, delete, suppress, or terminalize the 2465/333 rows. The sync remains separately unauthorized in the read-only audit turn. After approval, re-run the quiet gate, prove shadow parity/no duplicate without an exchange write, cut to queue, run fake-adapter crash rehearsal and one zero-inflight production restart, then complete Candidate B authority hardening, its focused/final full suite, exact gated deploy, and L2 observation."
...
| 6A | `phase-6a-durable-worker-command-boundary.md` | **in progress, stopped safe** 2026-08-22 — Candidate A `f257a93` remains global/queue/shadow; read-only audit found 2465/333 pending rows but the current notification bot is effectively disabled, so the next decision is whether to approve that frozen configuration for one real sync parity sample; queue/Candidate B/L2 observation not started |
