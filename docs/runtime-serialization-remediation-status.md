...
phase_6a_outstanding: "Execute the owner-approved Task 12A safe-sync supplement with focused RED/GREEN TDD. The migration-only reconcile-only HTTP shadow probe must preserve the no-header full sync contract, permit only normal database reconciliation plus Deepcoin reads, make exchange mutation and notification delivery unreachable, and be removed in Candidate B. After focused tests and one replacement-Candidate-A full suite, complete the production-copy probe rehearsal, exact-SHA gated deploy, frozen-outbox pre/post proof, and exactly one real probe. Do not call the existing ordinary sync under the no-exchange-write authorization. The 2465/333 outboxes remain unchanged from the last live audit. Queue cutover, crash drill, Candidate B, restart, and L2 observation remain unstarted."
