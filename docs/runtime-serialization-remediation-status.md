...
phase_6a_outstanding: "Resolve the newly proven incompatibility between the mandatory real sync sample and the owner's no-exchange-write constraint. The deployed sync route calls write-capable protection-rescue, backup-stop, and terminal-entry-cleanup lanes; production effective_position_management_liveness_v2_mode=live with 85 due rescue intents, 2 broad backup-stop candidates, and 17 cancellable unbound pending entry legs. Do not call sync under the current authorization. A future turn needs a separately approved approach that either accepts the route's existing possible exchange-write semantics in a proven safe window or changes the Phase 6A verification/design with focused TDD; do not temporarily disable live protection or invent a read-only substitute without explicit approval. The 2465/333 outboxes remain unchanged and notification_bot_effectively_enabled=false. Queue cutover, crash drill, Candidate B, restart, and L2 observation remain unstarted."
...
| 6A | `phase-6a-durable-worker-command-boundary.md` | **in progress, stopped safe at Task 12** 2026-08-22 — Candidate A remains global/queue/shadow and notification bot disabled; the approved real sync sample was not invoked because live liveness plus 85 due rescue intents, 2 backup-stop candidates, and 17 cancellable pending entry legs means the existing sync route cannot satisfy the no-exchange-write gate. Outboxes stayed 2465/333; queue/Candidate B/L2 observation not started |
