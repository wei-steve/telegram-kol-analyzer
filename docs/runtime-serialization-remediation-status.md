...
phase_6_task_3_unit_status: "COMPLETE locally and INSTALLED DISABLED. Three loopback-only hardened units for ports 8001/8002/8000 passed server-side systemd-analyze verify and are loaded, disabled, and inactive. Distinct non-login users plus telegram-kol-runtime exist; root-only 0600 role EnvironmentFiles contain 8 ingest keys, 45 worker keys, and 0 Web keys, with no Deepcoin key in ingest and no Telegram user-session key in worker/Web. The data tree gained shared-group access without removing prior permissions. Evidence: /opt/telegram-kol-analyzer/data/backups/phase6-split-units-disabled-20260823T003124Z.json."
...
phase_6_local_full_suite: "Final pre-split all-role Candidate c070896368da90f429f72b33934a5f6a7c0195d5: 6080 passed, 1 skipped, 17 warnings in 488.84s. No production code changed after this run."
phase_6_deployment_status: "Candidate c070896368da90f429f72b33934a5f6a7c0195d5 was fast-forward pushed and deployed twice with exact 40-hex EXPECTED_COMMIT through scripts/server_git_update.sh; both exits were 0. Postdeployment HEAD and installed/deployed/local updater SHA256 matched, telegram-kol.service was active, active_write_count=0, error-level journal empty, and modes remained global/queue/queue with semantic_review_enabled=false, auto_trade_enabled=true, and management_execution_mode=live."
...
phase_6_topology_status: "STOPPED AFTER FAILED SPLIT START AND SUCCESSFUL ROLLBACK. Exact Candidate c070896 is again running only as telegram-kol.service, active/enabled with PID 3260725 and default role=all. The three split units remain installed but are disabled/inactive. Modes remain global/queue/queue and semantic_review_enabled=false."
phase_6_cutover_attempt: "FAILED CLOSED at worker startup before Web or ingest was started. After the safe gate, the monolith stopped cleanly and the post-stop active-write check was zero. telegram-kol-worker then failed CLI validation because config/groups.yaml is 0600 root:root and unreadable by telegram-kol-worker; it never entered application startup. Restart=on-failure produced five bounded failed starts while the health loop waited, then the scripted error trap stopped split units and restored the monolith. No config permission was changed and no second cutover was attempted."
phase_6_isolation_test: "not run because worker startup failed before the split topology became active"
phase_6_rollback_status: "PASSED. Automatic rollback stopped ingest first (it had never started), stopped worker/Web, disabled all split units, enabled/started telegram-kol.service, and restored HTTP on port 8000. Independent evidence confirms exact c070896, monolith active/enabled PID 3260725, split units inactive/disabled, global/queue/queue, semantic_review_enabled=false, session owner only PID 3260725, active_write_count=0, active management/message backlog/worker inflight=0, WAL, quick_check=ok, and zero execution events since the cutover attempt. Complete direct Deepcoin post-rollback evidence exactly matched the pre-cutover position/order/fill/trigger identity summary: 2 positions, 0 regular orders, BTC/ETH pending triggers 12/3. Evidence: /opt/telegram-kol-analyzer/data/backups/phase6-cutover-failure-rollback-c070896-20260823T003819Z.json."
phase_6_stop_reason: "FAILED CLOSED 2026-08-23 at Task 7.3 because config/groups.yaml was not readable by the non-root worker unit. The rollback gate passed completely. Per the phase stop rule, no permission fix, second cutover, Web-load isolation, Task 8 monitor work, deployment, or post-split observation was attempted in this turn."
phase_6_outstanding: "Reclaim Phase 6 exclusively. Before another cutover, add a focused deployment/unit permission regression that fails for an unreadable shared --config-path, then make the minimum reviewed fix (expected operationally: grant telegram-kol-runtime read access to config/groups.yaml without exposing checkout secret files), run the affected focused tests, and update/install the reviewed unit/runbook material as required. Re-prove the safe window because the service restarted during rollback, then perform exactly one atomic cutover attempt. Only after a successful split may Tasks 7.4-7.5 verify natural-message queue ownership, management/SSE/loop health/SQLite/direct exchange behavior and Web-load isolation, followed by Task 8 monitor remediation and a new final full suite if production code changes. Preserve global/queue/queue, semantic_review_enabled=false, and all recognition/trading semantics."
...
| 6 | `phase-6-process-separation.md` | **in progress, stopped at Task 7.3 worker startup gate** — the fresh all-role L2 retry passed 5 natural messages, split users/envs/units were installed disabled, and the pre-cutover DB/exchange window passed. Worker then failed before app startup because `config/groups.yaml` was unreadable to its non-root user; automatic rollback restored exact `c070896` monolith with complete DB/exchange parity. Split units remain installed disabled; permission regression/fix, new safe window, cutover, isolation, and Task 8 remain outstanding |
