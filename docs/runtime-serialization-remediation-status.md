...
phase_6_outstanding: "Design and implement a durable worker-owned command/job boundary for all four Web exchange-authority routes without changing recognition, strategy, attribution, execution, or exchange-write semantics; then re-run the Task 1 authority gate and only if it passes continue with the Telegram session gate and Tasks 2-8. message_lock_mode must remain global and message_pipeline_mode must remain queue."
...
| 6 | `phase-6-process-separation.md` | **in progress, stopped at Task 1 authority gate** — SQLite three-process gate passed with 0/5400 SQLITE_BUSY, but four Web routes still reach worker-only exchange authority; no split/deploy occurred |
