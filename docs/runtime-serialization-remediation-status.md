...
phase_6_outstanding: "Phase 6A is now the canonical prerequisite and remains unimplemented. Complete its approved durable worker command boundary plan first; then re-run the Phase 6 Task 1 authority gate and only if it passes continue with the Telegram session gate and Tasks 2-8. message_lock_mode must remain global and message_pipeline_mode must remain queue."
...
| 6A | `phase-6a-durable-worker-command-boundary.md` | **planned** 2026-08-22 — approved design and complete TDD/L3 plan; no code, database, push, deployment, or exchange change yet |
