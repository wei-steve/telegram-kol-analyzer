...
phase_5_rollout_status: "dormant code deployed; production cutover deliberately not started because no post-deploy real message arrived in the observed shadow window, so unchanged live recognition/execution and an exactly-once boundary could not yet be proven"
phase_5_outstanding: "Observe at least one real post-deploy shadow message; prove shadow -> queue -> shadow with exactly one processing outcome at the transition boundary; re-check the safe window; enable queue; observe one complete real trading session; deliberately restart mid-traffic with a durable pending/claimed job and prove resume; verify missing/orphan/stuck/backlog plus loop lag; compare execution_events and Deepcoin regular/trigger order history directly for duplicate orders; only then complete Phase 5 and point to Phase 6."
