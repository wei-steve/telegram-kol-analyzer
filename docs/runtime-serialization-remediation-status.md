...
phase_6_task_3_unit_status: "COMPLETE locally, NOT INSTALLED. Three loopback-only hardened units exist for ports 8001/8002/8000 with distinct non-login users, a shared SQLite group, per-role EnvironmentFile, checkout-secret fallback denial, database-directory RW access, and Telegram session InaccessiblePaths on worker/Web only. The 45-test role/unit slice passed. Production still has none of the split units installed."
...
phase_6_topology_status: "STOPPED PRE-SPLIT at deployed c070896. telegram-kol.service is the sole active production unit and runs default role=all; telegram-kol-ingest/worker/web units are not installed or enabled. Modes remain global/queue/queue and semantic_review_enabled=false."
phase_6_isolation_test: "not run because the mandatory all-role L2 traffic gate failed before unit installation and topology cutover"
phase_6_rollback_status: "No rollback applied. Production remains healthy on Candidate c070896 in the unchanged monolith topology. The updater captured and restored the monolith correctly in both successful deployments. If code rollback becomes necessary, create a reviewed revert on top of the deploy branch and use the exact-SHA gated updater; no schema, database, setting, unit, or exchange rollback is currently required."
phase_6_stop_reason: "FAILED CLOSED 2026-08-22 after the first exact 30-minute all-role L2 observation received only 2 natural messages from 1 chat. The prior Phase 6A low-traffic waiver and Phase 6 queue-soak duration override do not waive this Phase 6 gate. No split unit was installed or enabled, no manual cutover occurred, and Task 8 was not started."
phase_6_outstanding: "Reclaim Phase 6 exclusively and rerun a fresh exact L2 all-role observation window on unchanged Candidate c070896. It must receive at least 5 natural messages within 30 continuous minutes; try to cover 2 chats and do not extend the window. Only after that gate passes may Tasks 7.2-7.5 install the units disabled, prove a new safe window, cut over atomically, verify topology/session/queue/management/SSE/SQLite/exchange behavior, run Web-load isolation, and then execute Task 8 monitor remediation with focused tests plus a new final full suite if production code changes. Preserve global/queue/queue, semantic_review_enabled=false, and all recognition/trading semantics."
...
| 6 | `phase-6-process-separation.md` | **in progress, stopped at Task 7.1 all-role L2 traffic gate** — prerequisite session ownership, role partition, hardened units, and dual-topology updater are implemented; exact `c070896` deployed twice under the unchanged monolith and technical gates passed, but the fixed 30-minute window had only 2 natural messages/1 chat, below the mandatory 5-message threshold. No split unit was installed or enabled |
