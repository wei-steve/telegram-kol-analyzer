...
phase_5_rollout_status: "dormant queue code remains deployed in global/shadow; the batch-119 race and exact zero-submission recovery have been repaired and fully tested locally, but the fix is not yet pushed, deployed, rehearsed on a production database copy, or applied to production, so queue cutover remains BLOCKED"
phase_5_outstanding: "Push and gated-deploy exact candidate a2ae02f only after a fresh production safe-window baseline. Verify batch 119 freezes recovery_required/management_close_submission_identity_missing without exchange changes; preserve an online backup and rehearse fingerprint-gated recovery on a separate production database copy; then, only with fresh complete exchange and zero-write evidence, apply once to production and verify the audited terminal result. After the blocker is safely resolved, prove shadow -> queue -> shadow with exactly one processing outcome at the transition boundary; re-check the safe window; enable queue; observe one complete real trading session; deliberately restart mid-traffic with a durable pending/claimed job and prove resume; verify missing/orphan/stuck/backlog plus loop lag; compare execution_events and Deepcoin regular/trigger order history directly for duplicate orders; only then complete Phase 5 and point to Phase 6."
...
| 5 | `phase-5-queue-consumer-takeover.md` | **in_progress** — global/shadow; batch-119 blocker fix tested locally, production recovery and queue cutover outstanding |
