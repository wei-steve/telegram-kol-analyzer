...
phase_6_outstanding: "Keep exact Candidate c099f28178c576522c26e911953f6784a491c739 split topology under normal monitoring. Before marking Phase 6 complete, restore the external DeepSeek context-resolution account/billing gate without changing provider authority or trading semantics; investigate and fix the ingest periodic iter_dialogs stall with focused RED/GREEN coverage if a code change is justified; then run the affected focused tests, one new final full suite only if production code changes, gated deploy, and one fresh fixed 30-minute L2 window requiring at least 5 natural messages, trying 2 chats, with successful unique jobs and natural management-path evidence. Do not re-enable semantic review or manufacture traffic/exchange writes."
...
| 6 | `phase-6-process-separation.md` | **in progress at final L2 acceptance** — exact Candidate `c099f28` is deployed as enabled/active ingest, worker, and Web processes; the monolith is disabled/inactive. Authority/session/credential isolation, Web-load isolation, monitor routing/timer, SQLite, duplicate detection, and direct exchange parity passed. The fixed 30-minute window had one 7.86-second ingest Telethon-dialog stall and only 2 natural messages/1 chat; both jobs were externally blocked by DeepSeek HTTP 402. Resolve those gates and obtain a fresh qualifying natural-traffic window before completion |
