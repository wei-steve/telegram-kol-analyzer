...
**6k 观察窗：判据未达成**（详见末尾 `phase-6k-window-closed`，此处只留结论）。
50 分钟 51 采样实质判据每轮全满足，唯独 `msgs >= 5` 全程为 0（收窗时当地 04:10，已核对是群静默而非采集中断）。
照实记未达成、不放宽、不重跑；**6k 是否 completed 由 P7 决定，不由该窗决定。**
...
- phase-6-release-gate-observability (2026-09-11, 会话 local_4a6676b0, **L1 达成；一个放开的闸门现在自己说出来**):
  上线 **`fdbf91da81878bc32d40288642842701b63fda7e`**，回滚参考 **`d48bba5f`**。
  四步部署四项全绿。全量 **8402 passed / 4 skipped / 0 failed**。
  **动机（A 线 2026-09-11 原话）**：*"一个放开的闸门对谁都不宣告。"*
  当天两条线为"某个闸门开没开"各停了一轮，最后是靠**读生产源码**settle 的——靠手。
  **改动**：新模块 `release_gates.py` 汇总四个决定"能否写交易所"的常量，两处出口——
  worker 启动日志一行指纹；`GET /api/runtime/release-gates` 返回每个闸门放开了哪些仓位、
  **以及放开它允许什么（用话写，不是只给常量名）**——一个 posId 出现时，
  读的人不该还要去翻常量才知道它现在允许什么。
  **生产实测（部署后逐条读，不是看提交）**：
  ```
  release_gates adopted_primary_backup_stop=1001125216121996,1001125216153672;
                break_even_full_exit=-;
                break_even_replacement=1001125216121996,1001125216153672;
                take_profit_limit_entry=1001125216121996
  ```
  同一行同时出现在 worker 启动日志与生产源码的直接求值里。
  **空闸门显示为 `-` 而不是省略**——"不在这一行上"与"放开了零个"不能长得一样。
  **完整性测试读源码找 `*_RELEASED_POS_IDS`**，不是比对手写清单：加第五个闸门而不上报即转红。
  变异验过（从报告里删掉一个闸门 → 红）。
  **观察脚本模板加 `gates_ok`**，与 `head_ok` 同形同后果：
  生产的闸门必须是开窗时那一组，否则采样不健康、窗口重置。
  **它读部署后的源码而不是 HTTP 端点**——否则 web 进程挂掉会被读成"闸门没变"，
  那是把"读不到"当成"没变"（硬性禁止第 4 条）。
  **L1 窗** 16:13:45Z ~ 16:29:17Z（931 秒）、**16 采样、零重置、零不健康、
  `head_ok`/`gates_ok`/`units_ok`/`reads_ok` 全程 1**、13 轮、`WINDOW_MET`；
  `gates` 全窗**只有一个取值**（未变）；三类写入计数全程 0。
  **这一步自己踩了一次它要解决的那类问题，记在 ARCHITECTURE**：
  起窗前我写的守卫 `ps -eo pid,args | grep -q '[/]root/observe_template.sh'` **当场误报**，
  终止了一个安全的动作——唯一匹配的是**检查命令自己的 `bash -c` 命令行**。
  `[/]` 括号只防 grep 匹配自己的 grep 进程，防不了外层 shell 的 argv。
  **机制对（真的 exit 1 了）、谓词错（用文本匹配回答身份问题）**；
  改用 pidfile + `kill -0` 后通过。**而这条规矩是我一小时前自己写下的。**
...
- pending-observations-registry (2026-09-11 起维护, 指挥会话要求, **每次收窗时顺带核对本节**):
  ...
  **(P4) `precancel` 场景的真实裸露时长**（6g，原 6g-2 之一）
  ...
  - 到达率：该路径（`effective_action = partial_close`）历史总计
    **17 个保护单 / 4 个仓位 / 4 个批次**——这个数字复核后仍然成立。
  - **"近 21 天 0 次"是错的，2026-09-11 复核时更正**：同一口径下近 21 天
    （2026-08-21 起）有 **16 条 precancel 事件 / 8 个保护单**，
    来自批次 155 与 156，**两者都在 2026-09-03**，就在窗内。
    原数字写下时显然用了别的时间边界或别的过滤；**它读起来像"这条路最近没动过"，
    而这条路最近动过两次**。到达率被低估，等于把一条其实可观测的项目登记成了不可观测的。
    记在这里而不是改掉了事：**一个从更窄的查询里出来的数字，读起来和覆盖全量的数字一模一样。**
  - 要核对什么：同批次内 `observed_at_wall` 的最早与最晚之差
    （这正是现在不可查、而 6g 仪表补上的那个量）。
  - **状态：未到达**——与上面更正后的到达率不矛盾：155/156 发生在
    **6g 仪表上线之前**，所以它们没有 `observed_at_wall`，
    历史批次答不出这个量。更正改变的是"还要等多久"，不是"已经有样本了"。
  ...
  **(P0) `protection_authority_shadow` 的到达率要从日志算，不能从表算**（2026-09-11 查清，指挥会话指派）
  ...
  - **状态：已回答。** 不再需要样本。
  **(P7) 6k 上线后前 5 个新仓位的首次保护写入，逐笔核对**（指挥会话 2026-09-12 裁定的附加条件）
  - **为什么要这一条**：6k 之后"放开一个仓位"不再需要部署，
    **谓词对未来每一个满足它的仓位都是常设许可**。前置样本（BTC 1 笔 + ETH 2 笔）
    证明的是"算得对"，**不是"这一类对象都安全"**——
    这两件事的差别正是 6j 收窄时那条异议的内容，它没有因为 6k 获批而消失，
    只是从"要不要做"变成了"做了之后盯什么"。
  - **触发条件**：6k 上线后，任何一个**新**仓位第一次由谓词放行而产生的
    止盈或备份止损写入。
  - **每次巡检核对什么（五项，任一不符即处置）**：
    (1) **价**——备份止损 = 主止损 × (1 − 20bps)，止盈 = 信号原文的止盈价；
    (2) **量**——备份止损 `whole_position`（不带 `sz`），止盈按计划档位；
    (3) **端点**——`set_position_sltp`；
    (4) **只增不减**——该合约条件挂单数只增不减，且新增数 = 预期数；
    (5) **主止损不动**——原主止损单仍在、价未变。
  - **不符时的处置（裁定指定，不是我自选的）**：
    **立即把该 posId 加进 `SOURCE_RELEASE_BLOCKED_POS_IDS` 并上报，
    不回滚整步。** 黑名单正是为这种"来不及部署的一小时"留的，
    而回滚会连带撤掉已经正确工作的部分。
  - **状态：未开始**（6k 尚未上线）。第 5 个仓位通过后，6k 记 `completed`。
  **(P5) `_restore_precancelled_protection_for_rejected_close` 是否真兜住过**（6g，原 6g-2 之二）
  ...
  - **状态：已查清（2026-09-11 16:41Z），答案是"这条路从未跑过"。**
    生产库全量：`strategy_management_protection_precancel` 事件 **44 条**
    （22 个保护单 × reserved/succeeded 各一）、**6 个仓位、6 个批次**
    （65、71、120、130、155、156，2026-07-26 ~ 2026-09-03）；
    六个批次**全部** `succeeded / all_position_protection_replaced`，
    六条 `strategy_management_legs` 全部 `succeeded` 且 `last_error` 为空；
    逐行拆开 `response_json.rows` 共 **19 行，`code` 与 `sCode` 全为 `0`、`sMsg` 全为空**，
    **零拒单**。而 `strategy_management_protection_restore` 事件
    **全库 0 条、从未出现过**。
  - **两件事必须分开说，不能合成一句"没问题"**：
    (a) **恢复路径从未被调用**，因为撤保护之后的平仓**从未被拒过**（19/19 成交回执 code 0）；
    (b) 因此**它有没有用、写得对不对，生产上零证据**。
    本仓库已经写下过"一个缺陷没有发生不是证据"——这里适用同一条：
    **`restore` 事件为 0 读起来像"兜底从没被需要过"，它同时也意味着"兜底从没被验证过"。**
  - **落点**：P5 从"待观测"转为**已回答**，但它回答的是"到达率"而非"正确性"；
    正确性只能靠单元测试与一次真实拒单，后者**不制造**（不得为验证而下单）。
...
- phase-6-dead-posid-retirement (2026-09-11, B 线，**已提交未部署，按裁定随下一次代码部署搭车，不单独起窗**):
  三个释放常量里的 posId 全部清空，**因为它们指向的仓位已经不存在了**：
  `ADOPTED_PRIMARY_BACKUP_RELEASED_POS_IDS`（原 `1001125216121996`、`1001125216153672`）、
  `BREAK_EVEN_REPLACEMENT_RELEASED_POS_IDS`（同两个）。
  `BREAK_EVEN_FULL_EXIT_RELEASED_POS_IDS` 与 `TAKE_PROFIT_LIMIT_ENTRY_RELEASED_POS_IDS` 本就为空。
  两笔仓位由用户于 **2026-09-11 14:42Z 手工平掉**（同一件事使 P1 的前提消失）。
  **退役的理由不是"用不上了"，是"读起来像还开着"**：一个指向不存在仓位的释放项，
  在源码里、在 `/api/runtime/release-gates` 里、在启动日志里，与一个真正开着的释放项
  **长得一模一样**；要分辨只能去交易所查那个仓位还在不在。而放开未来某个仓位
  无论如何都要改一次代码，所以留着它们不省任何成本，只制造一种"已获授权"的错觉。
  **退役不改变任何行为，但"从未被执行过"只对其中一个常量成立，不能合起来说**
  （生产库复核 2026-09-11 16:4xZ）：
  - `BREAK_EVEN_REPLACEMENT_RELEASED_POS_IDS`：**从未被执行过**。
    `position_mutation_intents` 里 `break-even:` 前缀**全时段 0 条**、
    `execution_events` 里 `break_even%` 动作**全时段 0 条**（无 TP1 成交 → 无 convergence，见 P1）。
  - `ADOPTED_PRIMARY_BACKUP_RELEASED_POS_IDS`：**被执行过，而且正是它该做的事**——
    intent 664 / 665（`trigger-backup-stop:349:601:...` 与 `:350:602:...`），
    2026-09-10 20:36Z 与 21:09Z，**两条都 `confirmed`**，即 6f 的那两笔真实备份止损。
    它是本阶段唯一**兑现过**的释放项。
  （同两个仓位上另有 671/672 两条 `tp-convergence:244:set:*`，是 A 线的限价入场止盈，
  不属于本节任何一个闸门；并列在此以免日后按 `pos_id` 查到它们时误记到这边。）
  **所以退役的是"已经兑现完、对象也已消失"的许可，不是"没用过的许可"** ——
  这两种情况写在同一句里会让前者读起来像后者。

- phase-6-released-gate-test-repair (2026-09-11, B 线，**上一条捅出来的**):
  清空 `ADOPTED_PRIMARY_BACKUP_RELEASED_POS_IDS` 让
  `test_a_released_position_is_not_held` 变成 `got empty parameter set` 的 **skip**——
  "一个读起来是绿的跳过"。改成由测试自己 monkeypatch 一个合成 id 之后，
  **再做变异检验：把闸门硬接成常闭（`if True:`）——4 passed，没咬住。**
  探针打出真实结果：`blocked / binding_or_leg_unavailable`——
  **这条用例从来没走到过那道闸门**。成因是夹具把入场腿的 `pos_id` 写死成 `"pos-1"`，
  而用例问的是另一个 id，`_plan_submission` 第一步就拒了；
  而断言只写了 `status != "shadow_ready_adopted_primary"`，**这个不等式被任何拒绝满足**。
  **修法**：(1) `_seed` 的 `pos_id` 同时写进腿和账本行，并在 docstring 里写明为什么必须如此；
  (2) 断言改成肯定式 `status == "ready"` 并核对它本该产出的 `slTriggerPx`；
  (3) **同一条用例里**用同一份夹具把闸门关上再跑一次，断言 `shadow_ready_adopted_primary`
  与 `primary_stop_adopted_from_exchange`——一个仓位、一处差异、两种结局。
  **修后三处变异全部转红**：闸门常闭（1 红）、闸门常开（2 红）、收养判定失效（2 红）。
  已记入 ARCHITECTURE §6 现有"因为夹具的原因被拒，不算拒绝"条目之下，
  并补上它真正的成因：**`!=` 天生分辨不了"没走到"与"走到了并放行"**。

- phase-6-posid-reuse-question (2026-09-11, A 线提问，B 线查证，**没有结论，这里记的是查到什么和查不出什么**):
  A 线问：释放名单里留着已平仓的 posId，万一交易所**复用**这个号，
  那就不是死数据而是"对一个陌生仓位的预授权"。**这个问题答不出，但值得记下边界。**
  - **没有任何一条生产数据支持"复用"**：全库 359 个 posId，
    没有一个呈现"出现 → 仓位关闭 → 隔一段再出现"的形状。
  - **但"号一直往上走所以不会回收"这个论证不成立**：按首次出现时间排序，
    相邻 358 对里 292 对递增、**66 对递减**。
    （多半是我们自己的锅——`execution_order_legs.created_at` 是我们写行的时刻，
    不是交易所开仓的时刻，收养与回填会把顺序打乱。**但它同样说明这个字段
    不能用来推交易所的分配顺序。**）
  - **查出一件相邻的事，和复用无关但更要紧**：我们自己的历史里有 **3 个 posId
    各自挂在两个不同的 strategy instance 下**，其中 `1001123942217592`
    横跨两个不同的群（`-1002370796392:3251` 07-05 与 `-1002409877375:8334` 07-06，
    同为 ETH short，相隔一天）。几乎肯定是**旧匹配器把同一个活仓位归给了两个信号**
    ——正是本阶段要拆掉的那类缺陷。**但仅凭这些行，分辨不了"交易所复用"与"我们误归属"**，
    所以两者都不下结论。
  - **实际落点**：**一个只按 posId 开的闸门，本身并不保证"就是这一个仓位"**
...
- phase-6j-deployed (2026-09-11 22:01:51Z, **五项判据首轮全部达成**):
  上线 **`eb05923b43a12a8420103c91ac40d8590ae66401`**，回滚参考 **`d8ae2550`**。
  全量 **8453 passed / 4 skipped / 0 failed**（含 A-16c 合并后的树）。
  **四步部署四项全绿**：生产 HEAD、共享分支、我的分支三处 sha **完全一致**；
  三个 unit 均 `active`（worker 427863 / web 427880 / ingest 427899）。
  生产端点实读的闸门行与期望**逐字相同**，`total_released=6`。
  **五项判据逐条（22:01:58Z 写入，22:02:38Z 实读核对）**：
  (1) 两仓各 1 张备份止损 **2479.03**（`sz=0`，即 whole_position）与
      1 张止盈 **2790 × 0.9** ✓；
  (2) 交易所条件挂单 **2 → 6，只增不减** ✓
      （`…241106`/`…241309` 主止损 2484、`…675686`/`…675915` 备份 2479.03、
      `…677523`/`…678020` 止盈 2790）；
  (3) **两张主止损 2484 仍在**，未被撤、未被改 ✓;
  (4) `position_mutation_intents` **恰 4 行、全部 `confirmed`**，
      **按 idempotency key 前缀归因**：`trigger-backup-stop:` 2 行（673/674）、
      `tp-convergence:` 2 行（675/676）、**其余 0 行** ✓；
  (5) **零 `close_position`、零 `cancel*`、零 `break-even:` 写入** ✓。
  账本侧六条 `position_protection_legs` 全部 `verified` 且带 `exchange_order_id`；
  `position_backup_stop_orders` 两行 `active`；**部署后零 incident**
  （闸门开着就直接提交，不再落 `backup_stop_shadow_ready`）。
  **观察器换了一版，理由必须写清楚，因为它形状上很像"把判据放松到能过"**：
  四笔写入发生在 **22:01:58Z**，而窗口 22:03 才开——
  所以 `new_intents == 4` **不是这个窗口能观测到的量**，它会读到 0 并报成失败。
  **判据 (4) 因此是在窗外用手核对的**（上面那一条，带归因），
  而窗口被改成回答**另一个问题**：放开之后**还会不会继续写**、它造出来的状态**守不守得住**。
  v2 的完成条件是 `new_intents == 0`——**在窗内多一笔写入就是失败，而在窗前那正是目的**。
  **这是改述不是放松**：每一条子句仍然可失败，且方向比原来更严（0 比 4 更难满足）。
  v1 的 3 条样本保留在 `/root/evidence/step6j-v1-partial/`，**不删**，
  好让日后有人能核对这次改述前后各自看到了什么。
  换版用的是**新文件 + 按精确 pid `kill`**，不是就地覆盖——
  bash 边执行边读脚本，覆盖会让运行中的副本从错误的字节偏移继续，且照样输出像样的内容。
  **一件与判据无关、但必须如实记的事**：这四个多小时里价格从 2575 跌到 **2509.06**，
  两仓均价 2560.48 / 2560.75，**现在都是浮亏**，但仍在两道止损之上（2484 / 2479.03）。
  止盈 2790 距现价约 11%。**判据达成说明"放开做了它该做的事"，
  不说明"这笔交易会赚"**——这两件事没有关系，不能因为前者绿就顺带暗示后者。
  **另：仓位行的 `slTriggerPx` 现在读作 2479.03**（备份价），不是 2484。
  这是 §4.8 记过的那条——**仓位行只反映最近一次写入**，
  一天之内它已经先后表示过主止损价、备份价与空值三种含义。
  **观察窗收窗（2026-09-11 23:22:27Z，`WINDOW_MET_NO_FURTHER_WRITES`）**：
  窗 **22:06:11Z ~ 23:22:27Z（4576 秒）**、**77 采样、零重置、零不健康**、
  `head_ok`/`units_ok`/`reads_ok`/`gates_ok` **全程 1**、
  **闸门指纹全窗只出现过 1 种**（即它没有在窗下被换过）、
  `msgs=10 / chats=3`（≥5 真实消息达成）。
  **逐样本而非取末行核对**：`new_intents`/`other_intents`/`close_intents`/
  `cancel_intents`/`released_closes` **全部 max=0 min=0**；
  `backup_ok_rows`/`tp_ok_rows`/`primaries` **全窗恒为 2**；`shrank` 恒 0。
  **收窗实读交易所（23:23:05Z）**：仓位 2 个、条件挂单 **6 张**，
  两张 2484、两张 2479.03、两张 2790×0.9，`code=0`——**与起窗时逐单相同**。
  **窗跑了 76 分钟而不是 30 分钟**，因为判据要 ≥5 条真实消息，前 30 分钟只到 0 条；
  **这是判据在起作用，不是窗口出问题**——时间到而消息不够时它继续等，
  而不是把"没有消息"当成"通过"。
  **浮亏如实记**：收窗时 lastPx **2511.24**，两仓各约 **-4.4 USDT**，
  仍在两道止损之上。**与本窗判据无关，但不写下来就等于只报了好消息。**

- phase-6i-did-not-fire (2026-09-11 23:3xZ, **只读核对发现：6i 已上线、运行正常、但结构上永远不会触发**):
  **指挥会话让我核对腿 582 是否被收掉。答案是：没有。**
  - 腿 582：仍 `status=pending`、`terminal_reason` 空，`updated_at` 停在 **17:51:32Z**
    （**早于 22:01 的部署**，即部署后从未被改动）。
  - binding 338：仍 `status=open`、`last_exchange_status=entry_order_pending`。
  - `conditional_entry_absent_from_exchange` 事故：**0 条**。
  - journal 里 `absent_conditional_entry` 字样：**0 行**。
  **根因（只读复现判定，零写入）**：在生产上用真实 client 重放这条腿的判定，得到
  `hold / history_not_exhausted / age_h=185 / searched_to_the_end=False`。
  逐级试页预算：`max_pages=5 → False`、`10 → False`、**`13 → True`**、`20 → True`。
  **`find_trigger_order_history_rows` 的默认 `max_pages=5`（500 行），
  而这个账户的条件单历史是 13 页 1176 行**——搜索每一轮都用尽预算、
  每一轮都如实返回"没找完"、模块每一轮都如设计地扣住。
  **代码完全正确，判据完全正确，而它永远不会成立。**
  **两层缺陷，第二层比第一层更该记**：
  1. **被调方的默认值，调用方从没想过。** 我调 `find_trigger_order_history_rows`
     时没传 `max_pages`，继承了一个为别的用途定的 5。
  2. **扣住了却什么也不说。** 没有事故、没有日志、没有计数——
     **"因为没找完而扣住"与"这条路根本没在跑"产生的证据完全相同，都是没有证据。**
     我能发现它，只是因为指挥会话让我去核对。**没人问的话，它会一直读起来像做完了。**
  **修法（6i-1，已写完，未部署）**：
  - `HISTORY_SEARCH_MAX_PAGES = 40` 并显式传给 finder。**取 40 不取 13**：
    13 是今天量到的历史长度，**把预算钉在一次测量上，等于下一次交易之后它再次静默失效**；
    40 留了约三倍余量，而且用尽时现在会打日志。
  - `format_verdict_for_log` + `QUIET_HOLD_REASONS`：**每轮都打**（照止盈扣住那条的做法），
    但对"这条腿本来就不归 6i 管"的原因保持安静，否则日志里每条腿每轮一行、
    真正被扣住的那条反而淹掉。
  **变异检验 6 项全红**：预算改回 5(1红)、预算钉成 13(1红)、不再传预算(2红)、
  把日志调用删掉(1红)、把该说的原因也划进安静集合(1红)、对所有腿都打日志(1红)。
  **第 4 项第一次是哑的**——我只测了 `format_verdict_for_log` 本身，
  **没有断言调用方真的调它**；删掉调用点，全绿。
  **这与 6i 原本的缺陷是同一个形状：规则没错，只是没人走到它。**
  补了两条用 `caplog` 的用例（真实 reconcile 轮次里那行必须出现 / 普通腿必须不出现）之后才咬住。

- phase-6j-day-after (2026-09-12 17:59:39Z 只读巡检, **一切照旧，无样本**):
  ETH-USDT-SWAP：**仓位 2 个仍在**（各 0.9，均价 2560.75 / 2560.48，lastPx **2524.93**，
  各约 **-3.2 USDT** 浮亏）；**六张条件单一张不少**
  （2484×2、2479.03×2、2790×0.9×2）；非条件挂单 0。
  BTC-USDT-SWAP：仓位 0、挂单 0。**无新仓位、无新挂单。**
  账本侧：**部署后 19 小时零新增 `position_mutation_intents`**；
  binding 352 的 `strategy_break_even_convergences` **0 行**；
  `break_even%` 事件**全时段 0 行**。
  convergence 248/249 由 `ready` 转 `submitted`。
  **P1 / P2 仍无样本**：2790 未到（现价 2524.93，距约 10.5%），TP1 未成交，
  保本分支自然也未触发。
  **仓位行的字段又换了一次含义，第三次记**：现在两仓的
  **`slTriggerPx` 读作空字符串**、`tpTriggerPx` 读作 `2790`——
  而交易所上**四张止损单（2484×2、2479.03×2）明明都在**。
  仓位行只反映**最近一次写入**，最近一次是止盈，于是它现在读起来像"没有止损"。
  §4.8 记过的"一天之内三种含义"，今天变成了第四种：**空值，而保护齐全**。
  **任何按仓位行判断"有没有止损"的代码都会在此刻判错**，
  能答对的只有挂单表——这也是 6b 绑定链要取代旧匹配器的同一个理由。

- phase-6k-decision-module (2026-09-12, B 线, **判定模块已写完，尚未接线、未部署**):
  新模块 `source_release.py`：**按属性放开，而不是按 posId 列举**。
  `evaluate_source_release(kind=...)` 两条来源判据——
  备份止损要 `evidence_source == "exchange_adopted_by_tu"`，
  止盈要 `entry_order_kind == "limit"`——**再各自叠加两条原提案没有的条件**：
  (a) binding 所属群 `trading_mode == "auto_trade"`；
  (b) 入场腿 `attribution_status == "verified"`。
  **为什么必须叠这两条**：`evidence_source` 与 `order_kind` 描述的是**订单**，
  **它们不说这个仓位是谁开的、开在哪个群**。手工开的仓位、或者只监听不交易的群里的仓位，
  两条来源判据都能满足——**而 `notify_only` 正是用户设来"什么都不做"的那种群**。
  这条是 6j 收窄时 B 线提的第三点，指挥会话把它定成了 6k 的前置判据。
  **未知一律不是放开**：`resolve_group_trading_mode` 把"没有 provider"、"群不在配置里"、
  "provider 抛异常"**三种原因收敛成同一个空字符串**——它们成因不同、后果相同，
  **没有一个是"这个群会交易"的证据**。而空字符串与 `notify_only`
  **用的是两个不同的 reason code**（`group_trading_mode_unknown` / `group_not_auto_trade`），
  因为一个是"配置成不交易"、另一个是"我们不知道"。
  **黑名单 `SOURCE_RELEASE_BLOCKED_POS_IDS` 默认空、且第一个检查**：
  从上往下读这个函数，"这个 id 到底能不能放开"在触及任何来源判据之前就有答案。
  **`describe_source_release()` 报的是条件不是 "all"**——
  一道没有列表可列的闸门必须改报它要求什么；
  **"all" 会是对最宽的许可给出的最不含信息的一行**，正是闸门可观测性要防的反面。
  ...
  **搜索到耗尽为止的 13 页 × 7 轮/分 ≈ 91 次/分，且只持续到这条腿在首轮被收掉为止**
  （收掉后它进终态，再也不进这个分支）。搜索**在历史耗尽时就停**，不会跑满 40 页。
  **查限流时我自己的 grep 先骗了我一次**：`grep -i 'rate limit\|429\|too many requests'`
  在过去一天命中 **61 次**——看起来像在被限流。逐条拆开后，
  **61 次全部是 posId/ordId 里恰好含有数字 `429`**，
  真正的限流字样 **0 次**。**一个过宽的匹配给出的假阳性，读起来和真问题一模一样**，
...
- phase-6k-window-closed (2026-09-12 20:10Z, **判据未达成：`msgs >= 5` 从未满足。照实记，不放宽**):
  窗 **19:20:06Z ~ 20:10:18Z（3012 秒，约 50 分钟）**、**51 采样**。
  **逐样本核对（不取末行）**：
  `head_ok`/`units_ok`/`reads_ok`/`gates_ok` **全程 1**；
  `new_intents`/`other_intents`/`close_intents`/`cancel_intents`/`released_closes`/
  `gates_unreadable`/`shrank`/`resets` **全部 max=0 min=0**；
  `orders_intact` 恒 **4**、`primaries` 恒 **2**、`backup_ok_rows`/`tp_ok_rows` 恒 **2**；
  **闸门指纹全窗只出现过 1 种**。
  **唯一未满足的一条：`msgs` 全窗 max=0**（`chats` 同为 0）。完成条件含 `msgs >= 5`，
  所以本窗**没有**达成，观察器以 `STOPPED_BY_OPERATOR_CRITERION_UNMET_msgs_0` 收尾
  （按精确 pid 停止，不是 `WINDOW_MET`）。
  **收窗实读交易所（20:10:36Z）**：ETH 两仓仍在，条件单 **6 张与起窗逐单相同**
  （2484×2、2479.03×2、2790×0.9×2）；BTC 无仓无单。
  **在记"未达成"之前先核对了"零消息"是群安静还是我们瞎了——这一步不能省**：
  全库最后一条消息 **17:38:41Z**；今日按小时到达 08 时 3 条、13 时 **33 条**峰值、17 时 2 条；
  ingest unit `active`。收窗时是 **UTC 20:10 = 当地 04:10**。
  **结论：群在夜里真的没有人说话，不是采集断了。**
  **所以要分开写"本窗证明了什么"与"本窗结构上证不了什么"**（A 线同日提出的区分）：
  - **证明了**：把两个列举式闸门换成谓词之后，**连续 50 分钟没有惊动任何已受保护的仓位**——
    那四张单一张没动、零新增 intents、零撤单、闸门从未在窗下被换过、每一轮都读到了闸门。
  - **证不了**：`msgs >= 5` 想代理的"系统在处理真实流量而不是空转"——本窗无样本。
    **以及更重要的**：谓词能否正确放行**下一个**仓位，本窗**从来就证不了**，那是 P7 的事。
  **我在起窗前写下过"若今晚消息稀疏导致收不了窗，照实记判据未达成，不悄悄放宽"。
  现在就是那个情形，按那句话记。**
  **这一条判据本身的毛病同日已由 A 线指出并记进 ARCHITECTURE §6**：
  `msgs` 的分母在系统之外，放进完成条件会让窗口变成"系统在等群里有人说话"。
  **本窗是那条教训在 B 线自己身上的实例，而不是例外。**
  **对 6k 的影响**：不回滚、不重跑窗口。6k 的实质判据全部满足，
  未满足的是一个系统外代理量；6k 是否 `completed` 本来就由 **P7** 决定，不由本窗决定。
