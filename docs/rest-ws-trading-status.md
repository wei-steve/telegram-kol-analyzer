...
- unmanaged-position-1001125178552543 (2026-09-10, 6-pre-4 会话 local_4a6676b0 只读发现，**已发现、未处置、转 A 线归因**):
  ...
  **【待验证，不作为 6-pre-4 的证据】** A 线一度推断"第 2 步的 POST 会
  `_invalidate_round_read_cache()`，所以第 3 步的 `list_positions()` 是真去交易所的新读，
  即持仓端点在订单侧已确知仓位存在之后仍未返回它 → 同一轮内两个端点互相矛盾"。
  **A 线随后自己提出了一个会推翻它的假设并正在查证**：leg 589 的 `pos_id` 在 01:23:03 那一刻
  可能还是按 `posId == ordId` 惯例**预填的条件单自身 id `1001125172997005`**，
  而条件单 id **永远不会**成为仓位 id——若如此，持仓端点并无滞后，
  它只是被拿一个不可能出现在持仓里的 id 去比对，"两个端点互相矛盾"随之不成立。
  （旁证：归属审计 3802 的 `prior_state` 是 `unassigned`，说明 01:23:10 认领**之前**没有归属；
  而我只读看到的 `pos_id=1001125178552543` 是认领**之后**的值。）
  **结论未定前本条不得引用为 6-pre-4 的支撑。** 6-pre-4 采用"快照比快照"的理由
  **不依赖**它——独立成立的那条是"本地账本会漂移"（本条 evidence 开头那个仓位本身即为实证）。
