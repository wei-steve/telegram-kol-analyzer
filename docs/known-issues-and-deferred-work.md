...
|---|---|---|---|---|
| **平仓后的保护投影仍可能保留 `verified`。** 文档分别记录了陈旧的 `position_protection_legs` 753–755，以及平仓后仍为 `verified` 的 protection-ledger 598–601；两者的准确表/行范围在修复前需重新确认。 | 当时交易所保护完整、后来持仓也已平掉，因此不是已证实的裸仓；但陈旧投影曾误伤部署门禁，未来管理/归属逻辑还可能阻塞或重复保护替换。 | `docs/ai-context-resolution-optimization-status.md`：**Read-only live-position protection audit and P0 checkpoint / Exchange position and current protection**、**R1 activation stopped at the fresh protection-ledger gate**。 | 下一次依赖本地保护簿记的部署门禁之前；或下一次真实持仓进入管理/保护替换前。 | 新鲜且完整的交易所持仓/普通单/逐合约触发单快照；确认当前权威表和准确目标行；定义平仓后的既有终结语义；数据库备份、单事务和回滚边界。 |
