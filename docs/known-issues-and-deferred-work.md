...
| **30 条 legacy execution_running 仍待生产修复，29 条副本演练通过、1 条排除。** 2026-09-05 重新逐条核验后，raw 14214 因 candidate 2129 / instruction 909 存在被排除；其已记录结果为 skipped / kol_or_group_auto_trade_disabled，未见执行提交证据，仍保留 running。其余 29 条全链无候选/执行记录、无新 attempt；副本单条 UPDATE 命中 29，running 30→1、uncertain 3→3，quick_check/FK 与回滚验证通过。**生产仍为 30/3，尚未 UPDATE。** | 这些旧行仍阻止 authoritative overwrite；涉及它们的 backlog expiry 仍可能被 execution_running_decision_present 拒绝。即使将来处理合格 29 条，剩余 running 14214 与 uncertain 14825/14843/14889 仍须保留各自保护，不能宣称全局门禁已解除。 | `docs/2026-09-05-legacy-execution-running-rehearsal.md`；早期调查仍见 `docs/2026-09-02-execution-running-read-only-audit.md`。 | 等所有者确认后仅处理已核验合格的 29 条；excluded 与三条 uncertain 单独处置授权，不混入批次。 | 生产前新鲜备份/SHA-256、逐条证据与前像复核、单事务精确影响 29、提交后 quick_check/FK/计数；不重跑消息，不写交易所，不改其他业务表。 |
...
2026-09-05T06:18:42Z 进一步反证：清除 monitor 专用 env 键并不使这两个旧 release
必然不可回滚。标准回滚保留当前 canonical unit，只重写通用 release drop-in；
两个旧目标均通过全树和实际有效配置的前瞻主导入证明，以 monitor 用户仅提供通用
身份变量导入旧身份模块也均验证成功。因此不按该理由退役，豁免仍保留。
详见 `docs/2026-09-05-historical-monitor-rollback-compatibility.md`；
该文同时单列部署容量门禁评估（冻结与授权消费之前检查操作后至少保留 8 GiB，
不能卡住必要回滚/解冻），尚未实现，不等同于备份清理授权。
