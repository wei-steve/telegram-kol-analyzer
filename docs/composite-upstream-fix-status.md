...
## 部署记录 2（2026-09-23，"TP1 已成交则不再减半"）

- 用户 2026-09-22 定规则（第一止盈单已自动成交 → 不再减仓、只移止损；未成交 → 减半并撤第一止盈单）并于 2026-09-23 批准部署。
- 候选 `a79a387b`（`5bd098fa` 变基到共享分支 `977af786` 之后；相关聚焦测试 408 通过，变基前全量 9540 / 0）。
- 零在途、无在仓仓位；`tg-deploy a79a387b…` → 五个服务 active、web 200、最近作业全部 succeeded、worker 错误行 0；自动交易开关未动（设置 `updated_at` 仍 2026-09-08）。
- **回滚 = `tg-deploy c028ddadc1e05127a9ec21d2171003827bfee1e7`**（交易代码等同 `fa72cf76`）。
- 首笔实盘样本补充核对项：组件一证据里 `first_stage_consumed_by_fill` 是否与交易所一致；为真时组件二 `planned_close_size=0`、无平仓写入、后续止盈一张未撤；新止损 `sz` 等于实时仓位。
