...
def _invalid_timestamp_sql(identifier: str) -> str:
    return (
        ...
        f"OR COALESCE(date(substr({identifier}, 1, 19), '+0 days') "
        f"!= substr({identifier}, 1, 10), 1) "
        f"OR COALESCE(strftime('%H:%M:%S', "
        f"substr({identifier}, 1, 19), '+0 days') "
        f"!= substr({identifier}, 12, 8), 1) "
