...
class MimoRecognitionRun(Base):
    """One bounded MiMo recognition run; it does not own execution."""

    __tablename__ = "mimo_recognition_runs"
    __table_args__ = (
        CheckConstraint(
            "run_kind IN ('v1_authoritative', 'v2_authoritative', 'v1_fallback')",
            name="ck_mimo_recognition_runs_kind",
        ),
        CheckConstraint(
            "status IN ('running', 'completed', 'failed')",
            name="ck_mimo_recognition_runs_status",
        ),
        CheckConstraint(
            "attempt_count >= 0",
            name="ck_mimo_recognition_runs_attempt_count",
        ),
        Index(
    ...
    retry_of_run_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("mimo_recognition_runs.id"), nullable=True
    )
    selected_attempt_ordinal: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )
    final_error_code: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
    final_error_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    became_authoritative: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    canonical_payload_fingerprint: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
    projection_fingerprint: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
...
class MimoRecognitionAttempt(Base):
    """Append-only provider-attempt detail for a MiMo recognition run."""

    __tablename__ = "mimo_recognition_attempts"
    __table_args__ = (
        UniqueConstraint(
            "run_id",
            "ordinal",
            name="uq_mimo_recognition_attempts_run_ordinal",
        ),
        CheckConstraint(
            "ordinal > 0",
            name="ck_mimo_recognition_attempts_ordinal",
        ),
        CheckConstraint(
            "duration_ms >= 0",
            name="ck_mimo_recognition_attempts_duration",
        ),
        CheckConstraint(
            "status IN ('completed', 'timeout', 'http_error', 'invalid_json', "
            "'contract_failure')",
            name="ck_mimo_recognition_attempts_status",
        ),
        Index(
    ...
    run_id: Mapped[int] = mapped_column(
        ForeignKey("mimo_recognition_runs.id"), nullable=False
    )
