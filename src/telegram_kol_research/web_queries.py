...
def _serialize_mimo_analysis(
    *,
    ...
    authoritative_runtime = _serialize_mimo_runtime(
        run=authoritative_run,
        runs=runs,
        attempts_by_run_id=attempts_by_run_id,
    )
    latest_call = _serialize_mimo_runtime(
        run=latest_run,
        runs=runs,
        attempts_by_run_id=attempts_by_run_id,
    )
    classification_run = authoritative_run or latest_run
    is_v2 = contract_version == "mimo-authoritative-v2" or (
        classification_run is not None
        and classification_run.contract_version == "mimo-authoritative-v2"
    )
    projection = _serialize_mimo_projection(
        is_v2=is_v2,
        is_historical_v1=classification_run is None,
        evidence=evidence,
    ...
    return {
        ...
        "version_label": (
            ...
            and authoritative_run.run_kind == "v1_fallback"
            else "MiMo v1结果"
            if result_format == "v1"
            else None
        ),
        ...
        # Compatibility alias: runtime is always the selected authority, never
        # an unrelated later rerun.
...
def _candidate_matches_mimo_intent(
    ...
    if action_kind in {"entry", "replace_entry"} and isinstance(strategy, dict):
        ...
        for field_name, candidate_value in (
            ...
            if field_name in strategy and not _structured_value_matches(
    ...
    if isinstance(parameters, dict):
        for field_name, candidate_value in (
            ...
            if field_name in parameters and not _structured_value_matches(
...
def _structured_value_matches(expected: object, actual: object) -> bool:
    if expected is None:
