...
【两套判据刻意不一致，不要自行调和】
- 上面【消息分类 message_classes】与【五类的判据】只决定 message_classes 这一个字段。
- 下面【新开仓识别】及其余既有条款只决定 recognition_result、lifecycle_event、instructions 等既有字段。
- 两者对“新策略”的门槛**故意不同**：message_classes 的 新策略 必须有止损价；recognition_result 的判据维持原样，仍是“止损/止盈/无效价/保护价/分批止盈计划至少有一个”。
- 因此一条“有标的、有方向、有入场价，只有止盈没有止损”的消息，正确输出是：recognition_result = 是策略，而 message_classes 不含 新策略（按【五类的判据】另行归类，通常是 策略管理 或 闲话）。
- 这种不一致是被观测的对象，不是需要修正的错误。**不要**为了让两者一致而改动其中任何一边，也不要在 reason 里把它当成矛盾来解释。
